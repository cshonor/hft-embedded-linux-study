...
→ 与 [19-Hennessy 体系结构](../../../../../17-computer-architecture/) · [CSAPP Ch6 存储器](../../../../../02-computer-systems/chapter-06-memory-hierarchy/) 同构
...
→ 完整表 [附录 F ASCII](../../appendix-F-ascii-table/) · [CSAPP §2.1.4 字符串](../../../../../02-computer-systems/chapter-02-representing-information/notes/section-2.1.4-字符串表示.md)
...
#### 10. 用 Python 自己验字节（WSL）

```bash
python3 -c "print('A'.encode('utf-16-le').hex())"    # 4100
python3 -c "print('中'.encode('utf-16-le').hex())"   # 2d4e
python3 -c "print('A'.encode('utf-8').hex())"        # 41
python3 -c "print('中'.encode('utf-8').hex())"       # e4b8ad
```
...
1. **`CHAR16` + `L"..."` = UTF-16** — UEFI 控制台 **只认这套**；编译时加 **`-fshort-wchar`**（见 [SETUP.md](../../SETUP.md)）。
...
→ [CSAPP Ch3 数据格式](../../../../../02-computer-systems/chapter-03-machine-level-programs/) · [02 附录 A 指令集](../../../../../17-computer-architecture/appendix-A-指令集原理.md)
