...
2. **UEFI 和 HTTP 类比？** — **成立** — 都只定规则；固件/OVMF 才是实现，EDK II 是官方完整实现套件。  
3. **`GetMemoryMap` 谁提供的？** — **主板 UEFI 固件**；EDK II 只提供 **头文件/封装**。  
4. **GNU-EFI 和 EDK II 都提供 Uefi.h，区别？** — gnu-efi 是 **小包装+GCC 链接**；EDK II 是 **完整构建树+库生态+可产整板固件**。  
5. **`efi_main` 和 `UefiMain`？** — 不同工具链 **入口符号名**；EDK 常用 **`UefiApplicationEntryPoint`** 再调你的 `UefiMain`。  
6. **`.inf` 和 `.dsc` 谁管全局？** — **`.dsc`** 平台顶层；**`.inf`** 单个模块。  
7. **OVMF 怎么来的？** — **EDK II 构建出的 UEFI 固件**，给 QEMU 当 `-bios`。  
8. **手写 `uefi.h` 和 EDK II `<Uefi.h>` 什么关系？** — **同源（UEFI 规范 / MdePkg）**；手写版是 **剥离 EDK II 框架后的极简子集**，用通用 clang/lld 编译。
