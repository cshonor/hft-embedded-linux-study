...
- HFT：评估是否可用 **PCID**、内核版本、mitigations 开关（与安全合规权衡）→ 与 [18-HFT ch05](../../../18-hft-engineering/chapter-05-操作系统内核极致调优/) 对照。

#### 3. Extended BPF（eBPF）

- 内核态 **安全虚拟机**：验证后运行，**可编程观测**（tracepoint、kprobe、uprobe、XDP/tc-BPF…）。
- 驱动 **BCC、bpftrace** 等 — Gregg 称「当前最重要技术之一」。
- **全书观测主线：** [Ch 15 BPF](../../chapter-15-bpf/) → [17-bpf-observability](../../../17-bpf-observability/)
