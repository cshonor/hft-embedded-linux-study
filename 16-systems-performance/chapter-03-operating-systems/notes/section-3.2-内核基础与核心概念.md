...
**调优直觉：** 热路径 **少 syscall、少拷贝、少阻塞**；内核旁路见 [15-DPDK](../../../15-dpdk/)、用户态栈。
...
**HFT：** 高频收包 → **硬中断 + NAPI + softirq** 占 CPU；`mpstat`/`perf` 里看到 `%soft` 高要往网络栈查。→ [06 内核网络 Ch 14 NAPI/RSS](../../../14-kernel-networking/)
...
- **isolcpus / 专用核**： housekeeping 与 hot path 分离 → [18-HFT ch05](../../../18-hft-engineering/chapter-05-操作系统内核极致调优/)
- Linux 里程碑：**O(1) 调度器** → **CFS**（完全公平调度，默认策略）

→ 深入进程/调度：[LKD 3rd Ch 3](../../../05-linux-kernel/00_Book_3rd_Notes/chapter-03-process-management/) · [Ch 4 调度](../../../05-linux-kernel/00_Book_3rd_Notes/chapter-04-process-scheduling/)
...
→ 精读：[06-linux-mm](../../../06-linux-mm/) Ch 3 页表/TLB/大页；[note-透明大页 THP](../../../06-linux-mm/chapter-03-page-table-management/notes/note-透明大页THP.md)
