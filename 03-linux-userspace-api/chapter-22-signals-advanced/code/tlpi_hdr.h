...
*                                                                         *
*  这是它的**最小替身**，只为本目录下「原书 Ch22 程序」（signal.c /       *
*  t_sigqueue.c / catch_rtsigs.c / t_sigsuspend.c / t_sigwaitinfo.c /     *
*  signalfd_sigval.c / demo_SIGFPE.c / sig_speed_sigsuspend.c）能零改动    *
*  编译而存在。                                                            *
*                                                                         *
*  原书真实文件有两层，本仓库都没有随附：                                  *
...
*       是纯字符串，不依赖 ename表 —— t_sigqueue/t_sigwaitinfo 等会触发。   *
*                                                                         *
*  ⚠️ 与其他章的同名替身**不通用**：                                     *
*     - 本 Ch22 那份含 get_num.h（t_sigqueue/t_sigwaitinfo/sig_speed_*    *
*       用 getLong/getInt）；t_sigsuspend 的 printSigMask 来自             *
*       Listing 20-4 的 signal_functions.{c,h}（跨章依赖，已随本目录）。   *
*     别跨章复制粘贴。                                                     *
*                                                                         *
...
#include "get_num.h"   /* 原书 tlpi_hdr.h 也含它；t_sigqueue/t_sigwaitinfo 用 */
