...
> ⚠️ **平台标注**：POSIX draft ACL（libacl + `system.posix_acl_*` xattr）是 **Linux 专有**完整实现；macOS 无对应常量/API。本章判定算法有全平台可跑的用户态复现（本目录 `c17_1_acl_algorithm.c`），API 类代码标注 Linux 复测平台（Pi5/ext4）。
