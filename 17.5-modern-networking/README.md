# 17.5-modern-networking

> 定位：**现代Linux内核（5.x / 6.x）网络子系统参考资料**
> 前置：`17-kernel-networking`（Rosen《Linux Kernel Networking》，基于3.x）
> 本目录存放现代内核网络栈资料，弥补 Rosen 书的大量过时实现；
> 学习完本目录材料之后，再回到 `17-kernel-networking` 做源码阅读与实操实验。
...
1. `17-kernel-networking`：理解内核网络栈需要解决什么问题，**不要照搬3.x代码实现**
2. `17.5-modern-networking`：学习5.x~6.x真正的现代网络栈实现
3. `18-dpdk`：用户态旁路网络，HFT 的主力收发路径
