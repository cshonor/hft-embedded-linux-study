...
| 5 | [busy-poll-mechanism](notes/05-busy-poll-mechanism.md) | 三代 busy polling：socket → SO_PREFER_BUSY_POLL → NAPI defer（内核栈延迟天花板） |
| 6 | [queue-steering-rss](notes/06-queue-steering-rss.md) | 多队列 / RSS / ntuple 流定向、中断绑核、irqbalance 与 managed_irq |
...
- **NAPI defer 是内核栈天花板**：`napi_defer_hard_irqs` + `gro_flush_timeout`（单位**纳秒**）
  零代码成本压到 2-5μs；p99 仍不达标才值得付出旁路复杂度 → [05](notes/05-busy-poll-mechanism.md)
- **行情流必须用 ntuple 钉队列**：组播四元组固定 → RSS 恒定落同一队列，不主动规划就队头阻塞；
  关 irqbalance、关 RPS/RFS、`isolcpus` 带 `managed_irq` → [06](notes/06-queue-steering-rss.md)
- **行情走 UDP 组播不是 TCP**：组播收包多出 `ip_route_input_mc()` 与 per-socket skb 复制，
  旁路后 IGMP 不再自动发出 —— 属**协议内部机制**，见
  [chapter-14/03-multicast-rx-path](../chapter-14-tcp-udp-internals/notes/03-multicast-rx-path.md)
...
- `12.5-modern-networking/chapter-06-af-xdp/notes/03-af-xdp-umem-layout.md`：AF_XDP 按队列旁路，配合 06 的流定向
- `12.5-modern-networking/chapter-14-tcp-udp-internals/notes/03-multicast-rx-path.md`：
  组播收包路径与旁路后 IGMP 失效（本章 05/06 的性能手段如何落到组播行情上）
