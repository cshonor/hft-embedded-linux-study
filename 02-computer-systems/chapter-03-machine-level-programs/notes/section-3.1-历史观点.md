...
| 嵌入式 | → [07-ARM64](../../../07-arm-architecture/)（奔跑吧 AArch64 · Smith ARM32） |
...
<details>
<summary>1. x86-64 是 CISC 还是 RISC？它和纯 RISC（如 ARM）在设计上有什么区别？</summary>

x86-64 是 **CISC**（复杂指令集），指令长度可变（1-15 字节），寻址模式丰富。但它内部解码为微操作（μops）执行，有 RISC 内核。ARM 是纯 RISC——定长指令、load-store 架构、简单寻址。HFT 中 x86-64 服务器和 ARM 嵌入式都需要了解，指令集影响代码生成和性能优化。
