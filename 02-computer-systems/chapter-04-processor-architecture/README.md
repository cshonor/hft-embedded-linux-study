# Ch 4 处理器体系结构 · Processor Architecture

> **CSAPP 3rd** · Bryant & O'Neill · **精读 🔴**（Part I）

> 本章定位：**CPU 怎么执行指令** — 用教学用 **Y86-64** 从单周期 SEQ 走到五段流水线 PIPE，理解 **流水线、冒险、分支预测**。真芯片比 Y86 复杂百倍，但 **stall、bubble、branch-miss** 的直觉来自本章。  
> **提醒：** 书中 **Ch3 真机汇编** 是 **x86-64**；Y86 是仿其风格的 **简化 ISA**，不是 Linux 上另一套生产标准。→ [Ch3 §3.3](../chapter-03-machine-level-programs/notes/section-3.3-数据格式.md)  
> **不是 Verilog：** 本章用 **HCL** 描述控制；Verilog 在 Harris Ch4（嵌入式 Linux 路线可跳）。路线对照 → [学习路线_CSAPP与Harris](../../../00-digital-logic-cpu/学习路线_CSAPP与Harris_Linux驱动.md)

---

## 小节笔记（按教材 4.1～4.5 拆分）

这 5 个 md 专讲 **CPU 硬件如何执行指令**，是 HFT 纳秒级优化的硬件理论基础。

| 节 | 笔记 | 学什么 |
|----|------|--------|
| **4.1** Y86-64 ISA | [section-4.1-Y86-64-ISA.md](./notes/section-4.1-Y86-64-ISA.md) | **纯教具**（非产品 ISA）：15 寄存器、极简指令；为 SEQ/PIPE 铺路；对照真 x86-64 |
| **4.2** HCL 与组合电路 | [section-4.2-HCL逻辑与组合电路.md](./notes/section-4.2-HCL逻辑与组合电路.md) | 组合(mux/ALU) vs 时序(寄存器/PC/内存)；HCL 纯教具；SEQ 电路积木 |
| **4.3** SEQ 顺序处理器 | [section-4.3-SEQ顺序处理器.md](./notes/section-4.3-SEQ顺序处理器.md) | 单周期五级 F→D→E→M→W，一条走完才下一条；资源闲置→引出流水线 |
| **4.4** 流水线原理与局限 | [section-4.4-流水线原理与局限.md](./notes/section-4.4-流水线原理与局限.md) | 多指令重叠提吞吐；流水线寄存器；抛出三类冒险 → 4.5 |
| **4.5** PIPE 与冒险 | [section-4.5-PIPE流水线与冒险.md](./notes/section-4.5-PIPE流水线与冒险.md) | **全书 CPU 硬件核心 / HFT 根基**：三类冒险 + 转发/stall/flush；软件如何少停顿 |
| 4.6 小结与模拟器 | 见 [§4.5 文末](./notes/section-4.5-PIPE流水线与冒险.md#46-小结与模拟器) | `ssim` / `psim` 等（选做） |
| **补充** 数字电路学习资料 | [section-补充-数字电路学习资料.md](./notes/section-补充-数字电路学习资料.md) | 总表 |
| **Harris ARM（独立书仓）** | [../../../00-digital-logic-cpu/](../../../00-digital-logic-cpu/README.md) | 已从本 notes 提出：一章一夹、一小节一文件 |

**学习顺序（严格从上到下，不能跳）：**

```
4.1 指令集 → 4.2 硬件电路 → 4.3 单周期 SEQ → 4.4 流水线概念 → 4.5 冒险与性能损耗
```

---

## 和第三章、HFT 的串联

| 章 | 层面 | 问题 |
|----|------|------|
| **Ch3** | 软件 | C → 汇编机器码：**指令长什么样**（栈帧、`call`、分支编码） |
| **Ch4** | 硬件 | 这些指令在 CPU 里怎么跑：**会踩什么性能坑**（stall、冲刷） |

无分支、少访存、`inline` 消 `call` — 底层理论多落在 **§4.5 流水线冒险**。

---

## 大白话 · 本章一条线

> **一条指令不是「一步做完」，而是分阶段流水作业；阶段多了，吞吐上去，但会互相踩脚。**

```
SEQ：取指 → 译码 → 执行 → 访存 → 写回   （每拍一条，简单但慢）
PIPE：五段并行填满 — 理想 CPI→1，冒险时 stall / bubble
```

### §4.5 三类冒险（必吃透 · HFT 元凶）

| 冒险 | 直觉 | 代码侧怎么躲 |
|------|------|----------------|
| **数据冒险** | 前后指令依赖 → 等数据 / 转发不够仍 stall | 拆依赖链、少紧挨的 load-use |
| **控制冒险** | `if` / `call`/`ret` → 冲刷流水线 | **无分支**、可预测分支、热路径 `inline` |
| **结构冒险** | 同时抢 ALU / 内存口 | 真机多靠复制硬件；写码侧少畸形争用 |

低延迟优化本质都是 **少让流水线停下来 / 被冲刷**。

**HFT 要带走的三件事（不必手画 HCL）：**

1. **分支预测失败** → 流水线清空，和 Ch3「不可预测分支」同一物理根因  
2. **数据冒险** → 真相关要停顿或转发；写代码时减少 **load-use** 距离  
3. **CPI / IPC** — `perf` 里 IPC 低，往 cache miss、分支、后端瓶颈想（→ [14-Systems-Performance Ch 6](../../19-systems-performance/chapter-06-cpus/)）

---

## 本章 Checklist

- [ ] 说出 Y86-64 程序员可见状态：PC、寄存器文件、CC、Stat
- [ ] 区分 **组合逻辑** vs **时序逻辑**（寄存器 + 时钟）
- [ ] 画出 SEQ 五阶段数据通路（文字级即可）
- [ ] 解释 **吞吐 vs 延迟**；理想流水线加速比上界
- [ ] 分类冒险：**结构 / 数据 / 控制**；各自典型对策
- [ ] 说明 **转发 (forwarding)** 解决哪些 RAW；何时仍须 stall
- [ ] 能口述：Ch3 看指令长什么样，Ch4 看指令跑起来卡在哪
- [ ] 知道 `csim`/`ssim`/`psim` 是本章配套模拟器（选做）

---

## HFT 精读捷径

```
理论必读：4.4 流水线局限 + 4.5 冒险 + 性能
与优化衔接：4.4 → Ch 5（循环展开、分支、ILP）
与观测衔接：branch-misses、cycles、IPC → SysPerf Ch 6/13
Y86/HCL/SEQ 细节：作业或第一遍扫读；复习抓 PIPE 冒险表
4.2 HCL：读懂即可，不必默写
顺序：4.1→4.2→4.3→4.4→4.5，勿跳到 4.5 硬啃
电路底子不够：补充「数字电路学习资料」→ Logisim 再回读 4.2–4.5
```

---

## 相关章节

- 上一章：[../chapter-03-machine-level-programs/](../chapter-03-machine-level-programs/)
- 下一章：[../chapter-05-optimizing-performance/](../chapter-05-optimizing-performance/)
- 真实微架构：[02-Hennessy](../../03-computer-architecture/)
- **数字电路补课：** [资料总表](./notes/section-补充-数字电路学习资料.md) · **[24-Harris ARM 独立书仓](../../../00-digital-logic-cpu/README.md)**
- 全书目录：[OUTLINE.md](../OUTLINE.md)
