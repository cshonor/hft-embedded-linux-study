...
3. **优化要有靶心** — 阿姆达尔：先打最慢的那段（→ 1.9，配合 [Ch 5](../chapter-05-optimizing-performance/)、[06.6-Systems-Performance](../../06.6-systems-performance/chapter-02-methodologies/)）

---

## 本章 Checklist

- [ ] 能解释 **上下文** = 比特的解码规则；举例 `0x41` 在编译器/CPU/内存/网络下的不同含义
- [ ] 能说明 **行情二进制协议** 如何体现「比特 + layout 上下文」
- [ ] 能画出：编译器 / 汇编器 / 链接器各产出什么（`.s` `.o` `a.out`）
- [ ] 能说清 **CSAPP 四步** 与 **编译 FE/BE** 的对应：`cc1`=FE+BE，链接不算编译器
- [ ] 动手做过：`gcc -E/-S/-c`、`gcc -v`、`objdump -d hello`（→ [1.2–1.3 实验](./notes/section-1.2-1.3-程序翻译与编译系统.md)）
...
- 性能方法论：[06.6-Systems-Performance Ch 2](../../06.6-systems-performance/chapter-02-methodologies/)
