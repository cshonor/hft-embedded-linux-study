...
把 2.4.1–2.4.5 串成一条管线，注意每一步的"断点"都在哪里：

```text
采样触发 ─▶ 抓栈（FP链/ORC/LBR） ─▶ 内核聚合 ─▶ 用户态符号化 ─▶ 呈现
             ▲断点1：无FP且无ORC     ▲断点2：缺符号表   ▲断点3：地址错位
               → [unknown] 帧          → 数字堆            → 归因到错函数
```

- **断点 1**（抓不到）：内核栈基本有 ORC 兜底，用户栈看编译选项——这是唯一能在编译期预防的断点
- **断点 2**（翻不出名字）：/proc/kallsyms 权限、ELF 被 strip——部署期可预防（保留 symtab、单独存 debuginfo）
- **断点 3**（翻错名字）：ASLR 基址算错、多版本库混装——运行期问题，交给 BCC/bpftrace 自动处理

防御顺序也由此定：**编译期保 FP > 部署期保符号 > 运行期用成熟前端**。三道防线都过，火焰图才是可信证据；任何一道缺失，图中 [unknown] 的占比就是数据可信度的折扣率。
...
<details>
<summary>4. 栈回溯管线的三个"断点"分别在哪一步？哪个能在编译期预防？</summary>

抓栈断点（无 FP 且无 ORC → [unknown] 帧）、符号化断点（缺 symtab/kallsyms 权限 → 纯地址）、归因断点（基址算错 → 名字张冠李戴）。抓栈断点可编译期预防（-fno-omit-frame-pointer），符号化断点可部署期预防（保留符号），归因断点交给成熟前端（BCC/bpftrace 自动处理 ASLR）。
</details>
