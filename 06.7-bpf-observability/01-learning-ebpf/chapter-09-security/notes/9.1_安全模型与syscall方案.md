...
## 9.4 真机佐证（lab08，树莓派 5 · 6.18.34 · 2026-08-30）

> 实验仓库：[ebpf-gate/labs/08-seccomp-lsm](https://github.com/cshonor/ebpf-gate/tree/main/labs/08-seccomp-lsm)

### 4.1 seccomp-bpf 沙箱实测（无 root，一次通过）

手写 classic BPF filter 禁止 `getpid()`（arm64 号 172），fork 子进程对照：

```
子进程(有 filter): getpid() = -1            ← ERRNO|EPERM 拦截
子进程其他 syscall 不受影响: getppid() = ...  ← 只禁一个，其余放行
KILL 版子进程被信号终止: yes (SIGSYS=31)      ← KILL_PROCESS 直接杀
```

一手坑与观察：

- **安装三件套**：`PR_SET_NO_NEW_PRIVS`（先放弃提权，否则非 root 不许装）→ `sock_filter[]`（classic BPF 短指令）→ `seccomp(SET_MODE_FILTER)`
- **classic BPF 跳转偏移是手数的**：架构检查的 jf 写错一位就越界，`seccomp: Invalid argument` 直接拒装——libseccomp 存在的意义
- **glibc 观察**：被 ERRNO 拦截后返回值 -1 确凿，但 glibc 的 getpid 包装没把 errno 置出来（打印为 0）——判定拦截看返回值，别只信 errno
- **`_exit()` 不走 stdio 清理**：子进程 printf 不 flush 输出会"神秘消失"——fork 实验的标准姿势：fork 前后各 fflush

### 4.2 BPF LSM 本机勘察（不可运行，但结论明确）

- 当前 LSM 链：`lockdown,integrity`（无 bpf）；启动参数无 `lsm=`
- `CONFIG_SECURITY=y` 但 BPF_LSM 未启用
- 双重门槛：即便加 `lsm=...,bpf` 重启，LSM BPF 程序强依赖 CO-RE（`struct file` 等类型来自 vmlinux.h），本内核无 BTF——`lsm_probe.sh` 的检测三件套：LSM 链、/proc/cmdline、内核配置

### 4.3 三代"拒绝"机制位置对比（全书实验收束）

| 机制 | 年代 | 挂钩点 | 本机状态 |
|---|---|---|---|
| classic BPF socket filter | 1992 | 抓包路径 | ✅ lab07 |
| seccomp-BPF | 2005 | syscall 入口 | ✅ lab08 |
| BPF LSM | 2020（5.7） | 安全钩子 | ❌ 需换内核 |
