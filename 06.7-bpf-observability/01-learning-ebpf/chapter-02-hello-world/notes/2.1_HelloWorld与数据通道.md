...
>
> 🧭 **先看 [2.0 随书 Python 例子与 BCC 全景注解](./2.0_随书Python例子与BCC全景注解.md)**：那里把「eBPF 内核层 vs BCC 框架层」的边界、Python/C 两侧的隔离、以及 BCC API 到底调用了什么（含 BCC 源码）讲清楚了。本节专注机制展开，不重复那部分。
