# 4.2 strace 实战分析（-c 统计 / -f 子进程 / -p attach / 阻塞与多余 syscall 定位）
...
> attach 的进程被 ptrace 接管时也会暂停，看完 `Ctrl-C` 退出 strace 进程即恢复。attach 前先 `ps -o wchan` 判断方向（见 2.2），再用 strace 精确定位卡在哪个 syscall、什么 fd。
...
- [2.2 attach 运行中进程](../../chapter-02-gdb-advanced/notes/02-attach-running-process.md)
- [03.6 模块导读](../../README.md)
