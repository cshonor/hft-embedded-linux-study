# 4.1 strace 入门（基本用法 / 输出格式 / 参数与 errno 解读）
...
> 配合 4.2 的 `-T`（每个 syscall 的耗时），时间戳能还原「哪个 syscall 之间卡了多久」。
...
> strace 追踪**系统调用**（syscall），不是库函数。`fopen` 是 glibc 库函数，不会直接出现；strace 里看到的是它底层的 `openat`（以及可能的 `fstat` 等）。同理 `fgets`→`read`、`malloc`→`brk`/`mmap`。库函数要用 ltrace 追（见 4.3）。
...
- [4.3 ltrace 库调用追踪](03-ltrace-library-calls.md)
- [3.2 加载 core 回溯](../../chapter-03-coredump/notes/02-load-core-backtrace.md)
- [03.6 模块导读](../../README.md)
