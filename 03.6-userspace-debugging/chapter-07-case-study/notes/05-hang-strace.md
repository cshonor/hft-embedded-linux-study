...
> ⚠️ **上面这段是「格式示意」，不是实测** —— 本环境的 strace 是 Windows 版二进制，
> 用不了（详见 5.1 的边界说明）。它想表达的因果是可靠的：**两个线程都停在
> `futex(...FUTEX_WAIT...)`**；但具体的 PID、地址、`2` 这个值都是占位的。
> 真正可实测的等价证据在下面「动手」一节（用进程内看门狗拿同一结论）。
...
## 动手：没有 strace，用进程内看门狗拿到同一结论（实测）

本节全篇依赖 strace——而 strace 要 attach 一个**已经在卡死的**进程，
这件事本身有个前提：**你得先知道它卡了**。生产环境里更常见的情况是
「服务半天没响应，运维才反应过来」。

`code/c7_1_trader.c` 里加了另一种做法：**让进程自己报警**。
`main` 里 `alarm(3)`，超过 3 秒没结束就由 SIGALRM 处理函数打印现场并 `_exit(4)`。

```bash
cc -g -O0 -pthread -Wall -Wextra -DBUG_HANG -o c7_1_hang code/c7_1_trader.c
./c7_1_hang
```
...
看门狗做了三件事，**每一件都能对应到 strace / gdb 的一个观察点**：

| 看门狗输出 | strace / gdb 的等价视角 |
|------------|------------------------|
| `feed 进度 0/200`、`match 进度 0` | **没有新 syscall 输出** —— 程序不再往前走了（对比「慢」：慢的程序还在持续出 syscall） |
| `g_book_lock: EBUSY` | `futex(0x…, FUTEX_WAIT_PRIVATE, …)` —— 所有人都在等同一把锁 |
| `feed 和 match 都卡住` | `thread apply all bt` 两个线程栈顶都是 `pthread_mutex_lock` / `__lll_lock_wait_private` |
| `_exit(4)` | ——（strace 看不到这个：它是主动退出，不是被信号打死） |

### 关键设计：心跳只在「真正推进」时才 +1

这是这份 demo 里最值得抄走的一个技巧。源码里 `feed` 有两条计数：

```c
g_feed_progress = i;      /* 每轮循环都更新 —— 它一直在涨，哪怕线程什么也没干成 */
```

而看门狗判定用的是**另一个**东西——只有真正拿到两把锁才 +1 的心跳
（`c4_2_deadlock.c` 里是 `g_hb`，这里是「进度」）。区别在于：

- 如果心跳**每轮循环都 +1**，那么卡在 `mutex_lock` 里的线程心跳也会停，
  但一个**空转**的线程心跳会一直涨 —— **你分不清「卡死」和「低效」**。
- 只有「真正完成了一轮工作才 +1」，心跳不动才等于**没有进展**。

**「心跳不涨」是区分死锁与慢的硬指标**，而这个指标必须靠「只在完成时打点」来保证。
很多线上服务的健康检查栽在这一条上：心跳用的是「循环计数器」而不是「完成计数器」，
于是卡死的进程照样在涨心跳。

### 对照：`_exit(4)` 和「被信号打死」完全不同

| 退出码 | 谁决定的 | 含义 |
|--------|----------|------|
| 139 / 134 / 136 | 内核（信号） | 进程被 SIGSEGV / SIGABRT / SIGFPE 打死 —— **意外** |
| **4** | **看门狗代码** | 有人主动判定「它死了」并退出 —— **判断** |
| 66 | TSan 运行时 | 检测到数据竞争后主动退出 |
| 1 | ASan/LSan 运行时 | 检测到内存错误后主动退出 |

**退出码本身就是一条线索**：拿到一个非 0 的退出码，先问「这个数字是谁给的」。
内核给的和程序自己给的含义完全不同——前者是「出事了」，后者是「我发现了事」。

### 这个看门狗和 strace 的关系：不是替代，是互补

- **看门狗**：常驻、无人值守、自动报警。适合「服务卡了但没人盯着」。
- **strace**：事后启动、信息全（能看到具体在等哪个地址、哪个 fd）。适合「已经知道卡了，去查到底卡在哪」。

现实做法是两者都上：**看门狗负责发现，strace/gdb 负责取证**。
