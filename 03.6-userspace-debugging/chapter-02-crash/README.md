...
本章 2.0 先划清 gdb 的**能力边界**（能干什么、与 ASAN 的冲突、看不到内核）；前半（2.1–2.3）学 gdb 基础——断点、单步、栈回溯，是**活体调试**（程序还在跑）；后半（2.4–2.6）学 coredump——崩溃后从内存快照**尸检**。两者互补：能复现就 gdb 活体调，偶发崩溃就抓 core 尸检。
