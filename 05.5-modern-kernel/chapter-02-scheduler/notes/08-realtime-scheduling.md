...
## 七、时间片真相：策略决定，不是调度类决定

**核心结论：调度类本身不决定有没有时间片——是调度策略决定的。**

| 策略 | 有无时间片 | 说明 |
|------|-----------|------|
| `SCHED_FIFO` | **无** | 拿到 CPU 一直跑到阻塞/让出；"FIFO 卡死"由 RT throttling 兜底（见二.1） |
| `SCHED_RR` | **有**（默认 100ms） | 只管**同优先级**轮转；更高优先级到来直接抢占，不管你还剩多少 |
| `SCHED_DEADLINE` | **不是时间片，是带宽** | `runtime/period` 限制"最多跑多久"，CBS 到配额会强制 throttle——像时间片的地方是"到点掐断"，但语义是保带宽供给+防超卖，不是同组排队轮转 |
| `SCHED_OTHER` | **无固定时间片** | CFS 按 vruntime 权重分 CPU **比例**，时间片是动态算出来的；固定时间片是老 O(1) 调度器的东西 |
| `SCHED_IDLE` | 同 OTHER | 是 fair 类正常成员，**不是"系统闲才跑"**——机器上只有它一个 fair 任务时照样跑满，只是权重最低（3），有人抢就只剩一口汤 |

### 抢占 vs 时间片（关键分野）

- **抢占**：地位不平等时的规则。跨调度类/不同优先级之间，高者就绪立刻抢，不看时间片
- **时间片**：地位平等时的规则。只作用于**同优先级、同策略**的任务之间轮流

通俗：**时间片 = "大家地位平等时，轮流分时间"；地位不平等，直接抢，不看时间片。**

> 精确化一句："高优先级就绪**立刻**抢占"在 RT/DL 层面成立；CFS **内部**的唤醒抢占有粒度阈值（`check_preempt_wakeup` 比较 vruntime 差距，差距不够大就忍住不抢，防抖动）——**地位悬殊立刻抢，地位接近看差距**。

### 四概念极简对照

| 概念 | 属于哪层 | 回答的问题 | 一句话 |
|------|---------|-----------|--------|
| 抢占 | 框架（类间）/ 优先级（类内） | 谁能抢走 CPU | 地位不平等，立刻抢，不看时间 |
| 时间片 | 策略（RR 的传统语义） | 同优先级怎么轮流 | 地位平等，轮流分时间 |
| vruntime | fair 类内算法 | 按权重分 CPU 比例 | 不是时间段，是记账本 |
| runtime/period | dl 类内模型 | 带宽配额 | 限"最多跑多久"，不是"到点换人" |
...
**Q: SCHED_FIFO 任务 A、B 相同实时优先级，A 在 CPU 上跑，B 就绪，会发生切换吗？换成 SCHED_RR 呢？**

<details>
<summary>答案（点开）</summary>

- **FIFO：不会切。** FIFO 没有时间片，A 不阻塞、不 sleep、不让出就一直占着，B 只能等
- **RR：会切。** A 时间片（默认 100ms）耗尽就排到同优先级队尾，B 上——但注意只限同优先级之间；更高优先级任务到来时不管时间片剩多少直接抢
</details>
