...
1. `std::variant` 把 tag 与当前对象绑定，并按活动类型执行正确的构造、拷贝、移动和析构。
   手写 `struct + tag` 依赖程序员维持二者一致，错误 tag、遗漏生命周期处理更容易导致未定义行为。
2. `std::visit` 以类型安全的 visitor 分派到当前备选类型，新增类型时编译期更容易暴露未覆盖处理。
   `switch` 需要手动读取 tag、转换载荷并维护分支，模板 visitor 还有更多内联优化机会。
3. 消息应语义明确、尽量不可变、尺寸有界，并携带完成处理所需的数据和关联标识。
   自包含可避免接收时再访问发送者的共享可变状态，也消除外部对象寿命与同步依赖。
4. `reply_to` 告诉处理者把结果发往哪个 mailbox，使请求者无需共享回调状态或等待全局响应队列。
   它支持多个并发会话和组件解耦；通常还需 correlation ID 匹配具体请求。
5. 小型值对象可直接复制或移动进队列，所有权和生命周期清晰，并有良好连续布局。
   裸指针会引入悬空、共享修改和内存回收问题；大消息可传受控句柄，但仍须明确所有权协议。
