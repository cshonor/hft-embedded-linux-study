...
1. `sender` 只暴露投递能力，便于把入口复制给多个生产者；`receiver` 拥有消费端与队列生命周期，通常不可复制。
   分离接口可防止发送方读取消息，也避免无意产生多个消费者破坏协议。
2. `wait_and_pop` 在队列空时阻塞，适合允许睡眠且希望节省 CPU 的线程。
   `try_pop` 立即返回成功或失败，适合轮询、事件循环或需要自行处理背压/其他工作的场景。
3. 在持有 `unique_lock` 时调用 `cv.wait_for(lock, duration, pred)`，谓词应检查“队列非空或停止”。
   返回值表明谓词是否满足；仍应以受锁保护的状态为准，不能把一次通知当成消息。
4. 阻塞队列可能发生内核睡眠、唤醒和调度抖动，难以满足热路径尾延迟要求。
   管理面吞吐较低且更重视节省 CPU，可以使用 mutex + condition_variable 的阻塞队列。
5. `unique_ptr` 表示 receiver 独占其队列实现，自动释放资源并禁止意外复制所有权。
   receiver 移动时可转移队列；sender 若需继续发送，应持有受控的非拥有引用或共享内部状态，取决于具体实现。
