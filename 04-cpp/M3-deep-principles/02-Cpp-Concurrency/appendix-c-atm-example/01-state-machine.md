...
1. ATM 的合法操作取决于卡、认证和交易阶段，状态机可显式描述“当前状态 + 事件 → 动作 + 新状态”。
   它限制非法转换，分离各阶段逻辑，并让流程更易测试、回放和审计。
2. 每个状态可实现为成员函数、状态对象或 `variant` 中的一种状态；该状态只处理允许的消息。
   处理事件后更新状态枚举、成员函数指针或状态对象，即表示一次状态转换。
3. 消息天然对应状态机的事件，接收者可按当前状态串行处理并更新私有状态。
   这样无需让发送者直接修改内部数据，降低共享同步和跨组件耦合。
4. `std::variant` 是带判别信息的类型安全联合体，会正确构造和析构当前备选类型。
   C `union` 不自动管理非平凡成员，tag 与内容也可能不一致；`variant` 可配合 `visit` 做受控分派。
5. 策略线程可将行情、成交、定时器和风控回复建模为事件，按 IDLE/READY/PENDING 等状态处理。
   它通过队列发送订单或风控请求，各组件保持单写者私有状态，从而得到可回放的确定转换流。
