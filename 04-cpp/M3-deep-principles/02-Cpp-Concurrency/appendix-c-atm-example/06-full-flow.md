...
1. 典型成功流是 `硬件线程(插卡/PIN/金额) → ATM线程 → 银行线程(验证/扣款授权) → ATM线程 → 硬件线程(吐钞/显示)`。
   硬件完成后再向 ATM 报告结果，ATM 更新状态并结束或继续会话；具体消息名以实现定义为准。
2. ATM 线程拥有会话和当前状态，银行线程拥有账户/授权状态，硬件线程拥有设备状态。
   不共享可变状态可让每份数据只有一个写者，组件间只交换消息，减少锁和不一致中间态。
3. 银行需要知道把授权结果发给哪个 ATM 会话或 mailbox，`reply_to` 提供返回路径。
   这解除对全局响应队列的依赖；配合请求 ID 还能正确关联多个在途请求。
4. 银行向 `reply_to` 发送拒绝/余额不足消息，而不是授权消息。
   ATM 转入拒绝处理状态，通知硬件显示原因，不发送吐钞命令，并按协议退卡或允许重选金额。
5. 两者都把输入、决策和执行拆成拥有私有状态的阶段，以有类型消息驱动状态转换。
   HFT 可对应为行情接收 → 策略 → 风控 → 下单/回报，强调单写者、异步回复和可追踪事件流。
