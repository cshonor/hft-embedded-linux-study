...
1. `state` 可声明为指向 ATM 某个无参状态处理成员函数的指针，例如 `void (atm::*)()`。
   运行循环通过 `(this->*state)()` 调用当前处理函数，类型系统保证只能指向匹配签名的成员。
2. 当前处理函数收到触发事件并完成动作后，把 `state` 赋为下一个成员函数的地址。
   下一轮事件循环便进入新状态；应先完成当前转换所需的不变量维护再更新状态。
3. `.handle<T>(fn)` 按消息类型组合处理器，避免集中式 tag 判断和巨大的 `switch`。
   每个状态只声明关心的消息，处理代码更局部且类型安全，但仍需明确未匹配消息和生命周期策略。
4. 每个线程拥有 receiver/mailbox，并把其 sender 交给其他组件；线程只向目标 sender 投递值消息。
   ATM 发认证请求给银行、银行回复 ATM，ATM 再命令硬件显示或吐钞，彼此不直接修改内部状态。
5. 策略可把行情、成交、风控结果和定时事件分派给不同状态处理函数。
   订单请求通过队列发送，策略状态由单线程串行更新，可减少锁并让事件回放与状态迁移可审计。
