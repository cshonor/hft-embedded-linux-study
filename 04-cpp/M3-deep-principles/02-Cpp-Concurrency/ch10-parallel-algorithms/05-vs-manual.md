...
1. 并行 STL 常只需在算法调用中加入执行策略；手写分块还要处理线程、边界、异常、同步和合并。
   差距取决于算法，但前者通常是一两行调用，后者是完整调度实现，不能用固定行数衡量质量。
2. 独立、规则、数据量足够大且标准算法可直接表达时适合并行 STL。
   需要固定绑核、NUMA 放置、定制调度/背压、跨阶段流水线或严格尾延迟时需要手写或专用 runtime。
3. 最主要原因是实现可自由选择分块、线程和调度，调用者难以约束最坏执行延迟。
   runtime 开销、共享线程池干扰、分配和同步也可能造成不可预测尖刺。
4. 先用 STL 可快速得到语义清晰、易验证的正确基线，并建立性能与结果对照。
   profiling 确认瓶颈后只替换必要部分，能减少手写并发范围和引入缺陷的机会。
5. 具体实现可能串行退化，也可能依赖 oneTBB 等后端或标准库自己的线程池。
   部署前要核对工具链实现、链接依赖、ABI/版本、运行时线程配置，并在目标机器验证性能与结果。
