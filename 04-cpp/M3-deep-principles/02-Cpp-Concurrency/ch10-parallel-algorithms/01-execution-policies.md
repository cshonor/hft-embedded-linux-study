...
1. `seq` 在调用线程中顺序执行；`par` 允许多个线程并行，但同一调用内的函数调用不彼此交错执行。
   `par_unseq` 还允许同一线程内 unsequenced/vectorized 执行，约束最强，不能依赖调用顺序。
2. `par_unseq` 的多个调用可能在同一线程上交错；一个 SIMD lane 持锁后，另一 lane 若等待同一锁，持锁 lane 未必能继续。
   `mutex::lock` 属于 vectorization-unsafe 的同步操作，调用会违反该策略对用户函数的要求并可能死锁。
3. `accumulate` 严格按迭代顺序做左折叠；`reduce` 允许分组和重排，再合并局部结果。
   并行需要这种重排自由才能分块计算，所以执行策略版本使用 `reduce`。
4. 要让任意分组和次序都得到等价结果，操作应满足结合律，并通常还需交换律及正确的单位/初值语义。
   减法可满足接口类型要求却不满足这些代数性质，结果依分组次序而变，不适合作为并行 reduce 操作。
5. 并行 STL 的分块、worker 调度和同步通常不由调用者精确控制，容易产生不可预测尾延迟。
   离线回测、盘后风险计算、大批量排序和报表等吞吐优先场景更适合使用。
