...
1. `transform`/`fill` 的各输出元素通常可独立处理；`reduce` 可把输入分块求局部值再合并。
   只要回调无共享副作用且归约满足所需代数性质，就容易安全并行。
2. `accumulate` 定义为按顺序左折叠，后一结果依赖前一累积值，因此不能任意重排。
   可用允许分组重排的 `reduce`，但操作必须对这种重排语义正确，浮点结果也可能不逐位相同。
3. 用 parallel scan：各块先求局部前缀和，再扫描块总和，最后把块偏移加回各元素。
   它增加若干遍操作，却把关键依赖链缩短并让大量操作并行，数据足够大时 wall-clock time 可更短。
4. `transform_reduce` 把逐元素映射与归约融合，避免生成中间容器并允许并行分块。
   例如计算两个向量的点积：先逐对相乘，再把乘积求和。
5. 先识别循环携带依赖，把可交换状态改成每线程局部状态并在末尾 reduce，或改写为 scan。
   无法安全分解的顺序状态机应保留串行，不应仅为并行而改变其语义。
