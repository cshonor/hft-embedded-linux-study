...
1. 若请求发生在注册之后，回调通常在首次成功调用 `request_stop()` 的线程中同步执行。
   它不是固定在被停止线程或独立调度线程上，注册者必须按并发回调来设计。
2. 构造 `stop_callback` 时若关联状态已被请求停止，回调会在构造它的线程中同步执行。
   因而构造过程本身可能调用用户代码，不能假设回调只在未来发生。
3. 若尚未开始，析构会注销回调，使其不再因该停止状态而执行。
   若另一线程正在执行该回调，析构通常要等待其完成，以保证析构返回后不再访问回调对象。
4. 注册一个生命周期覆盖等待过程的回调，在停止时调用 `cv.notify_all()`；等待谓词同时检查工作条件和 `stop_requested()`。
   通知不保存状态，所以必须依赖该谓词在持锁后重查，避免丢失唤醒和虚假唤醒问题。
5. 回调只应快速设置状态、通知条件变量或触发轻量的非阻塞唤醒。
   不应执行耗时 I/O、等待 worker、争抢复杂锁或抛出异常，否则会阻塞请求停止的线程并可能死锁。
