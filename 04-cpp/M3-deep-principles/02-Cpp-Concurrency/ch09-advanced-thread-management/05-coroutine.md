...
1. 线程是 OS 调度的执行单元，通常有独立栈并可在多核上并行；协程是可挂起的函数状态机，本身不等于线程。
   协程在显式挂起点让出执行权，恢复位置由 executor/事件循环等库层调度机制决定。
2. `co_await` 等待 awaitable，并可挂起后恢复；`co_yield` 产出一个值并挂起，常用于 generator。
   `co_return` 完成协程并把结果交给 promise，而不是执行普通函数的 `return` 路径。
3. 编译器把顺序写法转换成保存局部状态和 continuation 的状态机，异步完成时再恢复它。
   它消除了手写回调链的嵌套和状态传递，但异步 I/O、调度和生命周期管理仍由底层框架完成。
4. C++20 主要标准化语言变换与底层接口，没有统一 executor、事件循环、网络 I/O 或通用 task 类型。
   实际项目因此依赖库来定义返回类型、调度策略、取消、异常传播和协程帧管理。
5. 管理面、异步网络控制、日志或非关键 I/O 流程适合协程，因为它能简化大量等待型逻辑。
   纳秒级热路径通常不适合：挂起恢复、帧存储和调度可能引入难预测开销，固定同步流水线更可控。
