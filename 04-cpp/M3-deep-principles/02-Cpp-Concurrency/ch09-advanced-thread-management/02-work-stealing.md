...
1. owner 从一端操作可获得较好的 LIFO 局部性，并让大多数操作只触碰自己的队列。
   thief 从另一端取较老任务，既减少与 owner 的冲突，也更可能偷到能产生较多并行工作的粗粒度任务。
2. fork/join 会动态产生数量和耗时不可预知的子任务，本地队列保留缓存局部性，空闲 worker 又能窃取工作。
   单共享队列则让所有 worker 在同一同步点争用，递归任务越多越容易形成瓶颈。
3. worker 的本地队列为空时才尝试从其他 worker 窃取。
   受 victim 选择、竞争、任务时长和运行时调度影响，执行顺序及完成延迟通常不确定。
4. 窃取需要探测其他队列并进行同步，会带来额外缓存一致性流量和不可预测的调度抖动。
   HFT 热路径通常要求固定数据流、CPU affinity 和可控的最坏延迟，因此不采用这种动态调度。
5. 离线回测、批量风险计算、报表生成等任务量不均且重吞吐的场景适合使用。
   这些工作不在纳秒级关键路径上，能用动态负载均衡换取更高的整体 CPU 利用率。
