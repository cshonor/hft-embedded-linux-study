...
1. 核心组件是固定 worker 集合、任务队列、同步原语、停止状态以及提交/取任务接口。
   `std::function<void()>` 用类型擦除统一保存不同 callable，但要求目标可复制，并可能带来间接调用与动态分配。
2. `packaged_task<R()>` 包装调用并把返回值或异常写入共享状态，调用方先取得与之关联的 `future<R>`。
   `packaged_task` 只可移动，而常见任务队列存 `std::function`；用 `shared_ptr` 捕获后 lambda 可复制并延长任务寿命。
3. 谓词版本等价于在循环中检查条件，可正确处理虚假唤醒以及通知发生在等待前的情况。
   被唤醒后线程持锁重查“有任务或正在停止”，不满足就继续等待。
4. 通用线程池会有共享队列争用、调度与唤醒开销、任务分配，以及任务阻塞导致的吞吐和关闭问题。
   HFT 热路径更看重尾延迟和可预测性，通常采用预创建、绑核的固定流水线与预分配队列。
5. `std::function` 的类型擦除通常造成一次间接调用，较大的捕获对象还可能触发堆分配。
   这些开销会妨碍内联并扩大尾延迟，因此热路径常用模板 callable、固定任务类型或预分配存储。
