...
1. 静态分配开销低、局部性和执行时序可预测，但任务成本不均时容易失衡。
   动态分配可让空闲线程继续领任务，适合耗时不规则或无法预估的批处理，代价是队列同步和调度抖动。
2. 每个 worker 优先从自己的 deque 一端取任务，本地为空时才从其他 worker 的另一端偷取。
   owner 操作保持局部性并减少竞争，偷较老任务常能获得粒度更大的工作，同时与 owner 访问不同端。
3. 大多数 push/pop 发生在私有队列，避免所有线程争抢一个全局队列和同一 cache line。
   只有负载失衡时才执行跨队列窃取，因此同步成本更分散且能自动填补空闲核心。
4. 静态绑核能固定数据和线程归属，保留 cache/NUMA 局部性，并让调度路径与尾延迟更可预测。
   动态迁移和窃取会增加同步、缓存迁移及执行顺序变化，不适合严格实时的交易链路。
5. 盘后回测、组合估值、批量风控和报表等不规则、吞吐优先任务适合 work-stealing。
   它们能容忍调度波动，并可从自动负载均衡中获得更高总体利用率。
