...
1. 数据并行是多个 worker 对不同数据分片执行同类操作，例如并行计算多证券历史指标。
   任务并行是不同 worker 执行不同阶段或职责，例如行情解析、策略、风控和下单流水线。
2. 流水线让每阶段保持私有状态和固定职责，可并发处理不同消息并控制每段延迟。
   固定一对一阶段常用有界 SPSC 环形队列连接，需要扇入/扇出时再选择合适拓扑。
3. 阈值把小问题留在当前线程串行完成，避免创建/调度任务和 join 的成本超过计算本身。
   阈值太小会产生海量细碎任务、队列竞争、栈/帧开销和缓存局部性下降。
4. 对大量同构、独立元素执行相同计算适合数据并行；异构步骤、依赖图或不规则子任务适合任务并行。
   两者可组合，例如流水线某个离线阶段内部再对数据分块。
5. 根据物理核心/SMT、NUMA、I/O 队列、流水线阶段和实测负载配置，并为 OS/中断保留资源。
   线程超过可运行核心会增加上下文切换、缓存争用和调度抖动，不会自动提高吞吐。
