...
1. 全局变量在进程中通常只有一个实例；`thread_local` 让每个线程拥有独立实例，生命周期与对应线程关联。
   线程只访问自己的实例时无需为该对象互相同步，但把其地址共享出去或合并结果时仍需同步。
2. 热路径写线程私有缓冲不争用锁，也不在每条记录上执行系统调用，缓存局部性更好。
   满批后由各线程或专用消费者合并落盘，可摊薄同步/I/O 成本，但要设计容量、刷新和丢弃策略。
3. 具有动态初始化的 thread-local 对象通常在该线程首次 odr-use 前初始化，并在线程退出时析构。
   析构顺序、依赖已销毁的全局对象、长期线程占用大量 TLS，以及进程强制结束都可能造成问题。
4. 全局可变状态会让多个线程在无明确所有权下读写，容易产生 data race、锁序复杂和隐藏耦合。
   即使加原子也可能形成缓存热点；应明确单写者、不可变数据或受控同步边界。
5. 不共享可变状态从源头减少锁、原子 RMW 和 cache line bouncing，也简化内存序与故障隔离。
   HFT 因而更容易获得可预测路径和较低尾延迟，而不是依靠事后给所有数据加锁。
