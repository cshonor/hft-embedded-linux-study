...
1. 线程读到值 A 后暂停，其他线程把它改成 B 又改回 A；CAS 只比较当前位模式，因而仍会成功。
   但 A 背后的对象代次或链表关系可能已改变，所以“值相同”不代表状态没变。
2. 一个 pop 读出 `old_head` 后，另一线程可弹出并释放它，甚至让同一地址被复用后重新成为 head。
   前一线程随后访问 `old_head->next` 就可能解引用已释放或已换代对象，并错误地 CAS 链表。
3. tagged pointer 把地址与每次更新递增的版本号一起比较，即使地址回到 A，tag 不同也会使 CAS 失败。
   若要原子比较完整 64 位指针和 64 位计数通常需要 128 位 CAS；若能安全压缩字段，也可能用单个机器字实现。
4. 线程先把将要访问的节点发布到自己的 hazard slot，回收者只释放未被任何 slot 保护的退役节点。
   发布与首次读取之间节点可能已被移除，所以必须重读共享指针并确认仍相同，否则撤销保护后重试。
5. mempool 避免热路径通用分配并控制节点复用，tag 则区分同一地址的不同代次。
   组合后路径较固定、局部性好，但 tag 必须有足够位宽并正确处理回绕，mempool 本身并不能消除 ABA。
