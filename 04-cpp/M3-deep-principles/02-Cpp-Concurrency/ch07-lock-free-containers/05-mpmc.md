...
1. SPSC 的每个索引只有一个写者，普通 atomic load/store 即可发布进度；MPMC 有多个线程争夺同一入队或出队位置。
   后者必须用 CAS、exchange 或每槽序号等原子协议唯一分配位置并保持线性化。
2. 竞争者都对同一原子 cache line 做 RMW，所有权在 CPU 核之间不断转移，失败者还要重试并制造更多流量。
   mutex 在高竞争时可让等待者排队或睡眠，减少重试风暴，所以实际吞吐可能更高。
3. 本文展示的是 Vyukov 风格的 intrusive MPSC 算法，不是通用 MPMC：生产者用一次 `exchange` 取得前驱，避免 CAS 失败循环。
   优点是生产者路径短；代价是消费者只有一个，且生产者交换 head 后、链接 `next` 前存在必须处理的短暂不一致窗口。
4. 每个生产者独占一个 SPSC 的写索引，不再争夺全局入队原子，数据和索引也更容易保持缓存局部性。
   消费者轮询或归并多个队列会增加读取工作，但生产者数量固定时通常比热点 MPMC 更可预测。
5. MPMC 的 RMW 竞争、复杂内存回收和不可预测重试会扩大尾延迟，验证成本也高。
   HFT 常用固定拓扑拆成多个 SPSC、按 key 分片到单写者，或在非热路径使用有锁队列与批量提交。
