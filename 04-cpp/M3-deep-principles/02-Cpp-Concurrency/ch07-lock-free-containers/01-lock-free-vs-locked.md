...
1. wait-free 保证每个操作在有限自身步骤内完成；lock-free 只保证系统整体持续有操作完成；obstruction-free 仅保证线程独占运行足够久时完成。
   保证强弱通常为 wait-free ＞ lock-free ＞ obstruction-free，wait-free 最强也最难实现。
2. 多线程反复更新同一原子位置会造成 CAS 失败重试和 cache line 在核间迁移，浪费 CPU。
   高竞争 mutex 可把等待者阻塞或排队，反而减少无效工作，因此无锁不等于总是更快。
3. weak CAS 即使比较值相等也允许伪失败，这可适配某些硬件的实现方式。
   它适合放在本来就要重试的循环中；只尝试一次且伪失败难处理时通常用 strong。
4. 优先级反转是高优先级线程等待低优先级锁持有者，而中优先级线程又阻止后者运行。
   无锁避免了“等待锁持有者”这一经典形式，但不能保证高优先级线程不因 CAS 干扰、调度或资源竞争而饥饿。
5. 操作简单、竞争可控、不能容忍阻塞，且有成熟内存回收方案时适合无锁，例如固定拓扑的 SPSC。
   复杂不变量、高竞争、需要阻塞等待或团队难以验证内存序/回收时，优先选择锁和成熟容器。
