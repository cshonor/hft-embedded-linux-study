...
1. 标准库缺少统一的通用任务调度器、work-stealing、task arena/flow graph，以及成熟的并发容器集合。
   它提供的是可移植基础原语和算法接口，不规定完整并行 runtime 的控制能力。
2. `concurrent_hash_map` 封装了并发访问、细粒度同步、扩容和 accessor 生命周期，减少自行维护锁分片不变量的错误。
   它还可随 TBB 调度生态扩展；代价是引入库依赖且具体性能仍需按 workload 测量。
3. 动态不均匀任务、递归并行、复杂任务图和通用吞吐型计算更适合 TBB。
   只需基础同步、强调可移植性/少依赖，或需要自定义确定性调度和布局时，标准库更合适。
4. 热路径需要固定线程、绑核、预分配及可预测等待策略，标准原语和专用结构更容易精确控制。
   TBB 的通用 worker 管理和任务窃取以平均吞吐为目标，可能增加调度抖动与缓存迁移。
5. `std::execution::par` 是标准接口，标准并不规定后端必须是 TBB。
   某些标准库的 PSTL 实现会使用 oneTBB 等并行后端，也可能使用其他后端或退化执行，部署时应核对具体实现。
