...
1. 标准库提供线程、锁、原子、future 和执行策略等基础构件，但缺少统一的高级 task scheduler、work-stealing runtime、flow graph 和丰富并发容器。
   TBB 提供这些可组合设施，能自动管理任务粒度和负载均衡。
2. OpenMP 用编译指令给循环/区域加并行语义，改造数值循环快但依赖编译器 runtime，调度控制较粗。
   并行 STL 通过执行策略调用标准算法，接口可组合且类型安全，但算法范围和实现后端、可观测调度都受限制。
3. ASIO 适合大量异步 socket、定时器和控制连接，以少量线程驱动事件完成。
   HFT 中常用于管理面、网关或非最低延迟链路；核心行情接收可能采用内核旁路或专用网络栈。
4. 标准库基础原语加自研固定队列/流水线能精确控制分配、绑核、缓存布局和等待策略。
   TBB 的通用调度与 work-stealing 偏吞吐，可能带来热路径不需要的间接层和尾延迟抖动。
5. MPI 面向跨进程/跨节点的批量消息与分布式计算，通信延迟、序列化和集体操作不适合纳秒到微秒级本机热路径。
   它可用于离线集群计算，但不是低延迟撮合或行情流水线的典型工具。
