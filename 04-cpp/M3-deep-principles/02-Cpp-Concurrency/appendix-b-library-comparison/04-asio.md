...
1. Proactor 提交异步操作，操作完成后分派 completion handler；Reactor 通知描述符已就绪，再由应用执行读写。
   ASIO 向用户提供接近 Proactor 的完成式接口，在 readiness 平台上内部也可用 Reactor 机制实现。
2. 回调把后续步骤拆成多个函数并显式传递状态，链条长时控制流和错误处理容易分散。
   协程用顺序语法保留同样的异步挂起语义，局部变量、异常和取消路径更易组织，但不天然更快。
3. `io_context` 是 ASIO 的事件处理与 handler 执行上下文，关联异步 I/O、定时器和已投递任务。
   一个或多个线程调用 `run()` 驱动它；线程数量决定 handler 可否并发执行。
4. `co_await` 在操作未完成时保存协程帧并返回事件循环，完成事件再恢复到下一条语句。
   线程不会为该 I/O 同步阻塞，所以代码外观同步而执行机制仍是异步的。
5. ASIO 适合控制面连接、管理 API、日志传输和非关键 TCP/UDP 服务。
   极低延迟行情热路径通常采用专用 socket 调优、busy polling 或 DPDK 等方案；是否采用仍应以延迟目标和测量为准。
