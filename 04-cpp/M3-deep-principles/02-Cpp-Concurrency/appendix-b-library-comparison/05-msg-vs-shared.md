...
1. 共享内存可直接访问对象、少复制且单机延迟低，但需要同步并容易产生竞争、false sharing 和复杂所有权。
   消息传递隔离状态、边界清晰且易扩展，不过需要队列、背压以及数据复制或所有权转移设计。
2. 每个执行单元拥有自己的可变状态，减少全局锁和热点 cache line 的一致性争用。
   扩展时主要增加独立消费者和消息通道，而不是让更多线程竞争同一数据结构。
3. 行情接收、解析、策略、风控和下单可各自拥有状态，并通过有界队列传递不可变消息或所有权。
   这与固定阶段、单写者及可预测的数据流匹配，便于控制背压和测量每段延迟。
4. Actor 以私有状态和 mailbox 串行处理消息，不通过直接共享可变状态通信。
   HFT 的 SPSC 队列可作为两个固定阶段之间的高效 mailbox，但仅使用 SPSC 并不自动具备完整 Actor 语义。
5. 减少共享意味着更少的锁竞争、原子 RMW、cache line bouncing 和调度阻塞。
   单写者状态还能简化一致性协议，使执行路径和尾延迟更稳定；跨阶段通信成本则集中在明确队列边界。
