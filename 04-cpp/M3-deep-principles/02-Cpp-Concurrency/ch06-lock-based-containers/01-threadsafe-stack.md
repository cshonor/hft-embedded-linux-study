...
1. `top()` 返回到 `pop()` 之间锁已释放，另一线程可能弹走或替换栈顶。
   当前线程随后可能取得错误元素、重复处理，甚至对空栈调用 `top()`；必须把“取值并删除”做成一个锁内操作。
2. 先在锁内 `make_shared<T>(data.top())`，成功后才 `pop`；若构造 `T` 或分配控制块抛异常，栈保持不变。
   返回 `shared_ptr` 本身复制/移动不抛出这类 `T` 拷贝异常，避免元素已删除后向调用者传值失败。
3. 仍在。实现先执行 `value = data.top()`，只有赋值成功后才调用 `data.pop()`。
   若赋值抛异常，栈结构未修改；但调用方的 `value` 是否保持原值取决于 `T` 的赋值异常保证。
4. `empty()` 与 push/pop 并发访问同一容器，未同步读同样会构成 data race。
   const 成员函数要锁 mutex，应把 mutex 声明为 `mutable`，因为加锁改变的是同步状态而非抽象值。
5. 低频配置更新、管理面操作、启动/停机流程或低竞争对象池空闲链表可接受粗粒度锁。
   这些场景不在延迟关键路径，简单正确性通常比追求无锁更重要，仍应以竞争和尾延迟测量为准。
