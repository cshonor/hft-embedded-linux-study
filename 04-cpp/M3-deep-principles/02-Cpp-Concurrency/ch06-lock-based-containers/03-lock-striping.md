...
1. key 被映射到不同 shard，每个 shard 只保护自己的桶或数据，落在不同 shard 的操作可并行。
   单锁表把全部访问串行化，而分段把竞争限制在发生碰撞的那一部分。
2. 应结合并发线程数、热点分布、桶数、锁内工作量和内存开销通过基准选择。
   分段远多于桶数会产生大量无数据可保护的锁，增加空间和初始化/全表操作成本，却不增加并行度。
3. 先计算两个 shard；相同 shard 只锁一次，不同 shard 按稳定全局顺序加锁，或使用 `std::scoped_lock` 同时获取。
   整个 swap 必须在两把锁都持有时完成，才能保持跨分片操作的原子性。
4. 重要。`clear()` 与其他多分片操作若采用不同顺序，可能形成循环等待。
   应始终按 shard index 等统一顺序获取全部锁，并以逆序或 RAII 释放。
5. 可将每个分片的 `mutex` 换成 `std::shared_mutex`，读操作持 `shared_lock`，写操作持 `unique_lock`。
   但读写锁维护读者状态更重且公平性未保证，低竞争或写较多时普通 mutex 可能更快，应实测。
