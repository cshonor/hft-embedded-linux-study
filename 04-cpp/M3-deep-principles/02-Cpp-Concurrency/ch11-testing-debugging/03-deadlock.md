...
1. 四个必要条件是互斥、持有并等待、不可抢占和循环等待。
   工程上通常最容易通过统一锁序打破循环等待，或通过一次性获取多个锁消除持有并等待。
2. `std::scoped_lock` 对多个 mutex 使用与 `std::lock` 等价的死锁避免算法，再以 RAII 统一释放。
   标准只保证避免死锁，不规定具体算法；实现通常组合 `lock`、`try_lock`、失败回退和重试。
3. 给所有锁建立全局偏序并始终按同一方向获取，就不可能形成等待环。
   缺点是维护和审查成本高，动态锁集合难排序，还可能迫使扩大临界区或降低并发度。
4. 部分 TSan 配置可记录实际观察到的锁获取顺序并建立 lock-order graph，发现反向边形成的环。
   不必真的卡死，但相关相反锁序必须在运行中被观察到；该能力依平台和运行参数，不能替代设计审查。
5. 死锁中的线程阻塞等待且不再执行；活锁中的线程持续重试或互相礼让，却没有业务进展。
   活锁看起来仍占用 CPU、日志也在变化，普通“线程是否卡住”监控不易识别，需要检测进度指标。
