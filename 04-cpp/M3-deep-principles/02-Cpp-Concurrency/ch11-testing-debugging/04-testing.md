...
1. 串行测试主要验证输入到输出；并发测试还必须覆盖大量调度交错、同步顺序和活性属性。
   一次通过不能证明正确，需要结合压力、随机调度、sanitizer 和不变量检查。
2. 随机 `yield` 或短暂等待会改变线程到达同步点的相对次序，扩大竞争窗口。
   它只能提高覆盖概率，不能证明无 bug，而且过度 sleep 也可能掩盖真实生产时序。
3. 很多缺陷只在 push/pop、读/写、关闭/提交等不同操作交叉时出现。
   单一操作测试无法覆盖跨操作的不变量、线性化顺序和资源生命周期竞争。
4. 没有通用的固定时长；应按风险、操作量、线程配置和覆盖指标制定，并在 CI 中重复运行。
   一秒只产生有限交错，罕见竞争、计数溢出、长尾停顿和关闭竞态往往不会出现。
5. HFT 同时要求结果正确、进程稳定和极低尾延迟，而并发错误低概率但后果高。
   因此要投入单元/压力/故障注入/回放、TSan、性能回归及长稳测试；“50%”是经验目标而非标准常数。
