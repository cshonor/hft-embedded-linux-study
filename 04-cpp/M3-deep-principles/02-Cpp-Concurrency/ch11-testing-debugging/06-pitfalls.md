...
1. 未正确同步的普通指针 DCLP 本身有 data race；初始化还可概念化为分配内存、构造对象、发布指针。
   编译器或 CPU 可能让发布先于其他线程可见到完整构造，读线程据此访问未初始化对象；应用原子 acquire/release 或避免手写。
2. C++11 保证块作用域静态变量的首次初始化只由一个线程完成，其他并发线程等待其完成。
   初始化抛异常时视为未完成，后续调用可重试，因此 `static T instance;` 通常优于手写 DCLP。
3. 谓词形式会在持锁状态反复检查条件，自动处理虚假唤醒和通知先于等待发生的情况。
   正确性来自受同一 mutex 保护的状态谓词，而不是把 `notify` 本身当作可累积事件。
4. 原子引用计数只保护共享控制块的寿命；同一个 `shared_ptr` 句柄对象仍是普通可变对象。
   对该句柄并发赋值/读取需要锁或原子 `shared_ptr` 接口，被管理对象的数据也要单独同步。
5. detach 后线程可能比宿主对象活得更久，捕获的裸 `this` 变成悬空指针并导致未定义行为。
   对象由 `shared_ptr` 管理且继承 `enable_shared_from_this` 时，可在线程创建前捕获 `self = shared_from_this()` 延长寿命。
