...
1. 断点和单步会暂停部分或全部线程，改变锁竞争、超时、CPU affinity 和访问交错。
   原先很窄的错误窗口可能因此消失或变形，这正是 Heisenbug 的典型表现。
2. perf 基于硬件计数器和采样，可低扰动观察 CPU、cache miss、上下文切换和调用栈。
   它不在每次事件上做同步 I/O，更接近生产时序，适合定位 HFT 的热点与延迟来源。
3. 每个线程只写自己的预分配缓冲，热路径无需争用全局锁，也可避免逐条系统调用。
   由非关键线程批量收集或落盘，并接受有界丢弃策略，能显著减少日志对调度的扰动。
4. core dump 保存崩溃时的地址空间、寄存器和线程状态，可事后检查锁、对象和调用链。
   在 gdb 中可用 `info threads` 枚举线程、`thread apply all bt` 查看所有线程栈。
5. 先以模块、阶段或操作类型为边界，逐半禁用或替换为同步实现，并保持同一压力回放条件。
   观察问题落在哪一半后继续缩小，同时用 TSan/不变量断言验证，避免只靠时序变化误判。
