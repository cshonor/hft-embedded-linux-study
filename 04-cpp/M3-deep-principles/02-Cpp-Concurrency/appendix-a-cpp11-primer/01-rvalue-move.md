...
1. 左值通常表示有身份、可取地址的对象；右值通常是临时值或将亡值。
   非 `const int&` 绑定可修改左值，`int&&` 绑定右值；`const int&` 还可绑定右值并延长临时量生命周期。
2. `std::move` 只是把表达式转换为对应的右值引用类型，表示该对象可被移动。
   真正的资源转移由随后选中的移动构造或移动赋值完成；若没有合适重载，仍可能发生拷贝。
3. 标为 `noexcept` 让容器在扩容时可安全选择移动，同时维持强异常保证。
   若移动可能抛异常且类型可复制，`vector` 等通常会改用拷贝；不可复制时保证可能受限。
4. `std::thread` 不可复制但可移动，移动会转移底层线程句柄和 joinable 状态。
   `vector` 只需元素满足相应的 MoveInsertable 等要求，因此可以存放并在扩容时移动 `thread`。
5. 移动可转移缓冲区、消息和所有权句柄，避免深拷贝、额外分配及大块内存访问。
   对已预分配的数据通道尤其有利，但移动本身不保证零成本，具体仍取决于类型实现。
