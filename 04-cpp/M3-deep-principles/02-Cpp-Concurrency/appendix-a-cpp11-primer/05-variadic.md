...
1. 可变参数模板在编译期保留参数的数量和类型，类型安全且可参与重载、推导与优化。
   C 的 `va_list` 主要是运行时按约定读取无类型信息的参数，类型或数量不匹配容易产生未定义行为。
2. C++11 常用递归模板、逗号展开或 initializer-list 技巧逐项展开参数包。
   C++17 折叠表达式用指定运算符直接做一元或二元折叠，写法更短，并明确结合方向与空包规则。
3. `sizeof...(args)` 返回函数参数包 `args` 中的元素个数，是类型为 `std::size_t` 的编译期常量。
   对模板参数包也可写 `sizeof...(Types)`，它不计算各参数对象占用的字节数。
4. `std::thread` 会把函数和参数 decay-copy 到内部存储，再在新线程中以移动后的存储值调用。
   默认不是引用传递；需要传左值引用时应使用 `std::ref`/`std::cref`，并保证被引用对象活得足够久。
5. `std::forward<T>(x)` 按 `T` 的推导结果恢复调用者传入的左值或右值类别。
   对参数包逐项 forward 可避免无谓拷贝，并让下游正确选择复制、移动和引用重载。
