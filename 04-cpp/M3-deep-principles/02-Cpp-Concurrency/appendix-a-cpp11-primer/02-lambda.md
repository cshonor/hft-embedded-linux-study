...
1. 值捕获在创建闭包对象时复制当时的值，之后外部变量变化不影响该副本。
   引用捕获在闭包中关联原对象，调用时访问当前值，因此必须保证对象寿命和并发同步。
2. `[=]` 按需值捕获 odr-use 的自动变量，`[&]` 按需引用捕获；两者对 `this` 等细节还受语言版本规则影响。
   并发中显式值捕获通常更易保证寿命，但并非天然线程安全；共享可变对象仍需同步。
3. 初始化捕获允许闭包成员由任意表达式初始化，并可拥有原作用域中不存在的新名字。
   它可把 `unique_ptr` 等 move-only 对象移入闭包，明确地把资源所有权交给异步任务。
4. `[this]` 捕获的是指针，不会延长对象寿命；detach 后对象可能先析构。
   线程随后通过悬空 `this` 访问成员会产生未定义行为，应改为值快照或受控的共享所有权。
5. 每个 lambda 都有具体闭包类型，模板接收时编译器通常能内联其 `operator()` 并传播常量。
   函数指针是间接调用且不能携带捕获状态；不过无捕获 lambda 转成函数指针后同样可能有间接调用代价。
