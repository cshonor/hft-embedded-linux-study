...
1. `const` 只表示对象不能经该名字修改，初始化值可能到运行期才知道；`constexpr` 变量必须可用于常量表达式。
   因而保证编译期常量资格的是 `constexpr`，但具体表达式是否在编译期求值仍取决于使用语境。
2. C++11 的非构造 `constexpr` 函数体基本只能包含单个 `return`，不能写普通循环、局部变量等。
   C++14 放宽为可含局部变量、分支、循环和对局部对象的修改，只要常量求值路径仍满足规则。
3. 能。`constexpr` 函数既可参与常量求值，也可用运行期参数像普通函数一样执行。
   只有在要求常量表达式的上下文中，且实参与执行路径均合法时，结果才必须在编译期得到。
4. `if constexpr` 的条件在编译期求值，未选择分支会被丢弃，不参与当前实例的实例化要求。
   普通 `if` 两个分支都须是可编译代码，只是在运行期决定执行哪一边。
5. 编译期查表把初始化计算移出运行期，可减少启动或热路径计算，并避免首次使用时的同步。
   表可放只读存储并便于优化，但仍占代码/只读数据空间，访问时仍受缓存行为影响。
