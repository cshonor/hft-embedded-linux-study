...
1. `std::move(x)` 本质上就是一次 `static_cast<T&&>(x)`——把实参无条件转换成右值引用，**它不移动任何东西、不产生任何代码**（零运行时开销）。真正发生"移动"的是后续的重载决议：变成右值后，重载决议会选中移动构造/移动赋值，由它去交接资源。所以 `std::move` 的正确理解是"我允许你搬走它"的**许可标记**，而不是移动动作本身。

2. `std::move` **无条件**把实参转成右值——用于"确定不再需要这个对象、要转移所有权"的场合（右值引用形参、即将销毁的局部对象）。`std::forward<T>(x)` **有条件**：只有 `T` 是（非引用的）值类型、即原始实参是右值时才转成右值，若 `T` 是 `T&`（原始实参是左值）则保持左值——用于万能引用形参中"原样保留实参的值类别"的完美转发。一句话：右值引用用 `move`，万能引用用 `forward`。对万能引用误用 `move` 会无条件把左值也变成右值，导致调用方的对象被意外掏空。

3. 标准规定：被移动后的对象处于**有效但未指定**（valid but unspecified）的状态——可以安全地析构或赋新值，但不能对它的内容做任何假设。实际上 `std::string` 被移动后通常变成空串（常见实现是交换指针，源变空），但这是实现细节、**不是标准保证**，不同容器/不同类型表现不同。正确的做法是：移动之后要么不再使用该对象，要么先给它重新赋值。可以安全调用的操作仅限"无前置条件"的那些（析构、赋值、`empty()`、`clear()` 等）。

4. 什么也不会发生——移动请求会**静默退化成拷贝**。如果没有移动构造函数，重载决议就会选中 `T(const T&)`（拷贝构造），因为 `const T&` 也能绑定右值。所以 `std::move` 从不会让程序出错，只是"可能白费力气"。对 `std::array` 这类没有移动优化的类型尤其明显：它内部是逐元素移动，实际代价与拷贝相当。

5. 有严重 bug：`x` 是万能引用，`process(s)` 传入左值 `s` 时 `T` 被推为 `std::string&`，`x` 的类型是 `std::string&`。接着 `std::move(x)` 无条件把它转成右值，`store()` 选中移动构造，**把 `s` 的内容搬走了**。之后 `std::cout << s` 输出的未指定内容（通常是空串），而调用方完全没预料到自己传进去的字符串被掏空——这正是"对万能引用误用 `move` 而非 `forward`"的经典事故。修正：
```cpp
template<class T> void process(T&& x) { store(std::forward<T>(x)); }
```
用 `forward` 后，传左值时保持左值（走拷贝，`s` 不变），传右值时才移动。
