...
| 延迟预算 | `epoll_wait` 唤醒 + 调度 ≈ 微秒级——热路径依旧嫌慢 → [06](./06_NonBlockingIO.md) 的 busy poll → [14 DPDK](../../13-dpdk/) 内核旁路 |
| `SO_REUSEPORT` | 多核各自持独立 epoll，连接按哈希分片到核，消除锁与惊群——低延迟网关标配 |
| 观测 | 每秒事件数、事件循环延迟分布用 [16 BPF](../../15-bpf-observability/) 直接量 |
...
- [04 Netcat（select 版）](./04_Netcat.md) · [13 内核网络](../../12-kernel-networking/) · [13.5 现代网络](../../12.5-modern-networking/) · [14 DPDK](../../13-dpdk/)
