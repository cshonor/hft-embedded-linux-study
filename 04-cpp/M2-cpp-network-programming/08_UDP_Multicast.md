...
| 丢弃点 | 触发条件 | 观测手段 |
|--------|----------|----------|
| 网络路径 | 拥塞、CRC 错误 | 抓包两端对比（[12.5 Wireshark](../../11.5-wireshark-packet-analysis/)） |
...
1. `recvfrom` 返回 **1000**（它返回的是实际拷给你的字节数），**多出的 500 字节被直接丢弃**——UDP 是数据报语义，一次 `recvfrom` 消费**整个**数据报，截掉的尾部不会留到"下次再读"，而且**没有任何 errno**，是静默截断。所以接收缓冲必须 ≥ 最大数据报（以太网 MTU 1500 → 通常取 2048；jumbo 环境更大）。要探测对端到底发了多大，可用 `recvmsg` + `MSG_TRUNC`（Linux 返回完整长度）或先用 `ioctl(FIONREAD)` 查队列上下一个数据报的大小。

2. UDP 的 `connect()` **不发任何报文**，只是给 socket 打上对端标记，之后行为变化有四：① 可以用 `send`/`recv` 而不必每次填地址；② 内核**过滤**：来自其它地址的数据报直接丢弃（免费的安全过滤器）；③ 能收到**异步 ICMP 错误**，例如对端端口不可达 → 后续操作返回 `ECONNREFUSED`（未 connect 的 UDP 收不到这类错误）；④ 省掉每次发送的路由查找，热点路径更快。可再次 `connect` 改对端，或用 `AF_UNSPEC` 解除。单对端场景（行情/订单单播）应该 connect：更省、更安全，还白送错误回报。

3. 三个丢弃点及观测命令：
   - **网络路径**（交换机/链路丢包、CRC 错误、IGMP snooping 配置不当）：两端同时抓包 `tcpdump -i eth0 -w a.pcap 'udp port 50000'` 后对比；交换机侧看端口丢包计数。
   - **网卡/驱动 ring buffer 溢出**（中断跟不上收包速率）：`ethtool -S eth0 | grep -iE 'drop|miss|no_bufs'`、`netstat -i`、`cat /proc/net/dev` 的 drop 列。
   - **内核 socket 接收缓冲满**（最常见，"交换机统计正常但应用层缺号"）：`cat /proc/net/udp` 的 **drops** 列、`netstat -su` 的 `receive buffer errors`、`ss -unm` 看 `skmem` 与 `sk_rcvbuf`。
   另有 IP 分片重组失败：`netstat -ip`。定位顺序建议：先看 `/proc/net/udp` drops 与 `netstat -su` → 再看网卡计数 → 最后两端抓包对比。应用层则用帧里的 sequence number 做 gap 检测，再通过 TCP 重传通道补。

4. 组播 **TTL 默认是 1**：报文到第一跳路由器就被丢弃，**出不了本网段**——现象是"同网段能收、跨网段收不到"，常被误判成代码 bug。跨网段/跨机房必须显式设 `IP_MULTICAST_TTL`（≥ 需要经过的跳数，生产常设 1~几，不要盲目设大，否则报文会在网内乱窜）。`IP_MULTICAST_LOOP` 决定本主机发出的组播是否**回环给同主机的接收者**（Linux 默认开启 = 1）：**同机自测/同机多进程联调必须开**，否则本机接收者收不到；生产环境若同机还有其它接收者且不希望它们重复收到，可以关（关之前先确认没有本机接收者依赖它）。

5. 两个角度：
   - **延迟确定性**：UDP 无连接、无重传、无拥塞控制、无队头阻塞，一个数据报就是一个数据报，**延迟抖动小、行为可预测**；TCP 丢一个包就要等重传（RTO 或快重传），后面所有数据都被队头阻塞，抖动是毫秒级的。行情要的就是这种确定性。
   - **可靠性职责**：行情"最新的才有用"，丢一帧可以由**应用层**用序列号检测 gap、走重传通道（通常是 TCP）或 A/B 双馈互补，把可靠性放在应用层按需实现，比让 TCP 强制重传已经过期的行情划算得多；而且组播天然一对多，一份数据全网段共享带宽。
   订单反过来：**每一笔都必须可靠、有序、不重复**地送达，丢单/重单是资金事故，TCP 的重传、排序、流控、拥塞控制正好把这些职责从应用层拿走；订单是低频点对点，建连握手/RTT 的代价可以用**连接预热**消化。一句话：**行情要确定性 → UDP（组播）+ 应用层可靠性；订单要可靠性 → TCP**。
