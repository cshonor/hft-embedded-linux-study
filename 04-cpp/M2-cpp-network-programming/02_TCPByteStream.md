...
1. 可能是 **1 到 4096 之间的任意值**，只保证"不少于 1 字节、不超过 4096 字节"。它取决于：① 该连接接收缓冲里**此刻已到达且序号连续**的字节数（受对端发送节奏、网络分段影响）；② `read` 的请求长度 4096；③ 是否还在持续到达（慢启动/cwnd 限制下可能只有 1~2 个 MSS，即 1460 左右）。TCP 是字节流，**一次 `read` 的边界与 `send` 次数毫无关系**——一万次 1 字节 `send` 完全可能在接收端被"粘"成几次大 `read`。

2. prependable bytes 让"在已有数据**前面**补内容"可以**原地完成**，不用挪动数据、不用重新分配。典型场景：网关收到一帧 payload 后要转发给下游并加上自己的 4 字节长度头（或会话头、纳秒时间戳），可以直接把长度写到 prependable 区，然后把 `[prepend 区, 数据区]` 这一整段一次性 `write` 出去——零拷贝、零 `memcpy`。若没有这段空隙，就得先 `memmove` 腾位置或拼两个 buffer（`writev` 也行，但 muduo 的写法更省一次系统调用）。

3. 因为长度值来自**对端**，攻击者/出错的客户端发一个 0xFFFFFFFF，`resize` 一下就要 4GB，进程直接 OOM 被打死；即使不 OOM，也会为一个永远等不齐的"超大帧"长期占住连接和缓冲。这是典型的"输入驱动的分配"攻击面。对策：长度必须按协议上限校验（如 `kMaxFrameLen` = 64MB），超限即判协议错误并断开连接；同时限制单连接的待处理字节数与解码速率。

4. `EINTR` 表示这次 `read` 被**信号打断**，一个字节都没读走，连接本身完全正常，属于"重试即可"的瞬态错误。若当错误处理（返回 -1 并断开），会在收到 `SIGCHLD`、定时器信号、profiling 信号时**误杀正常连接**。所以循环里必须 `continue` 重新 `read`。补充：Linux 上阻塞 `read` 若已经读到了部分数据再被信号打断，会返回已读字节数而**不**报 `EINTR`——这也是 `readn` 能正确记账的原因。

5. 不一定。Nagle 只在"有小于 MSS 的未确认数据"时才攒包，批量传输时每次 `write` 都是满 MSS 的大块，Nagle 根本不触发，所以**吞吐基本无差别**——测吞吐本就不该关 Nagle（会人为凑出更多小报文、头部开销变大）。反过来，**小块 + 请求-响应**场景（撮合回报、交互式协议）关掉 Nagle 明显更好：避免 Nagle 与对端延迟 ACK 叠加造成的最多 ~40ms 停顿，延迟大幅下降，而报文数本来就是 1:1，没有额外成本。结论：测吞吐用大块，测延迟（和线上小消息）必开 `TCP_NODELAY`。
