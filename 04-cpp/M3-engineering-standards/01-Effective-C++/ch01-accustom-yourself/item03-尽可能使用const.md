...
与 [条款 2](item02-尽量以const、enum、inline替换%23define.md) 衔接：用 `const` 取代宏常量；本条把 `const` 用到指针、迭代器、参数、返回值和成员函数上。
...
← [条款 2：const 替换 #define](item02-尽量以const、enum、inline替换%23define.md) | [条款 4：初始化 →](./item04-确定对象被使用前已先被初始化.md)
