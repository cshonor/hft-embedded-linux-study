...
**文件夹 10** · [返回嵌入式支线](../HFT-READING-ROADMAP.md#六嵌入式-linux-支线10–14)
...
10-arm-architecture/
├── README.md                          ← 本页（模块总览）
├── arm32-asm/              ← Smith *ARM Assembly Language*（v4T / v7-M）
│   ├── OUTLINE.md
│   ├── chapter-01 … chapter-18
│   ├── appendix-* · glossary · references · code
│   └── _scripts/
└── aarch64-practice/        ← 《ARM64体系结构编程与实践》（AArch64）
    ├── OUTLINE.md
...
```
Smith Ch2–8、13、16、18（可选/压缩，在 arm32-asm/）
        ↓
奔跑吧 Ch1–23（AArch64 主书，在 aarch64-practice/）
```
...
| 后续模块 | 需要汇编直觉 |
|----------|--------------|
| [20 构建](../11-embedded-boot-build/) | U-Boot 启动早期、内核 `head.S` |
| [21 驱动](../12-device-drivers-dt/) | 中断入口、原子指令 `ldxr`/`stxr` |
| [04 LKD](../07-linux-kernel/) | 对照 x86 `syscall` ↔ ARM `svc` |

**顺序：** Smith 精读章（或跳过）→ [**奔跑吧 OUTLINE**](./aarch64-practice/OUTLINE.md) → [20 Mastering Embedded Linux Programming](../11-embedded-boot-build/)。

---

## 《ARM64体系结构编程与实践》（摘要）

📋 全文裁剪 → [aarch64-practice/OUTLINE.md](./aarch64-practice/OUTLINE.md) · [导读](./aarch64-practice/README.md)
...
| [03 Hennessy](../03-computer-architecture/) Ch2 | MESI — ARM 同样适用 |
| [08 MikanOS](../05-os-from-scratch/mikanos/) | UEFI/x86 启动链 — 与 ARM **概念平行** |
...
- [ ] 知道设备树为何取代 hard-coded 寄存器（→ [21](../12-device-drivers-dt/)）

**下一章：** [20 嵌入式 Linux 构建](../11-embedded-boot-build/)
