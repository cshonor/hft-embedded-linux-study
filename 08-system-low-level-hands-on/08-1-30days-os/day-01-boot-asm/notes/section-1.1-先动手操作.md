...
2. 找到 **「Stefan Weil provides binaries…」** 一句 → 点红色链接 **`64-bit`**（64 位 Windows 用这套；Day 1 的 `qemu-system-i386` 在此安装包里）
3. 在镜像站文件列表里下载最新 **`qemu-w64-setup-….exe`**（文件名含 **`qemu-w64-setup`**）
...
**安装：** 双击 `qemu-w64-setup-….exe` → 路径建议 **`D:\DevTools\QEMU`** → 勾选 **Add to PATH**（若有）→ 完成。新开 **cmd**：
