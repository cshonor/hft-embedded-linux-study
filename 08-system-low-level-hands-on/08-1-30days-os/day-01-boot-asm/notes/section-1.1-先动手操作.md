...
**不必装 VMware / VirtualBox** — Day 1 只需 **QEMU** 这一只轻量模拟器，模拟 **x86 PC 从软盘 A: 引导**。
...
| 推荐 | 说明 |
|------|------|
| **官方预编译包** | [qemu.org/download#windows](https://www.qemu.org/download/#windows) → 进入 **Stefan Weil** 的 Windows 镜像页 → 下载文件名含 **`qemu-w64-setup`** 的 **`.exe`**（最新版即可） |
| **tolset 自带** | 原书工具包里常有 `qemu-system-i386.exe`，与后续 Day 共用 |

**不要**用来源不明的「QEMU 便携版」第三方打包 — 官方 **`qemu-w64-setup`** 是预编译 Windows 安装程序，带安装向导，安全性有保障。

**安装：** 双击 `qemu-w64-setup-….exe` → 安装路径建议 **`D:\DevTools\QEMU`**（与 HxD 一样避开系统盘权限）→ 勾选 **Add to PATH**（若有）→ 完成。新开 **cmd** 验证：

```cmd
qemu-system-i386 --version
```

默认安装且未加 PATH 时，可执行文件通常在：
