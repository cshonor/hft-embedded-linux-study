...
| | |
|---|---|
| **本章干什么** | **加载独立 ELF 应用**（如 **cat**）；命令行参数；与内核 **分离地址空间** 的前奏。 |
| **全书作用** | **用户程序载体** — 内核不再包办一切逻辑；暴露 **链接基址 / 加载地址** 问题 → 直接引出 Ch19 分页。 |
| **← 前置** | [Ch17 FAT / Block I/O](../chapter-17-filesystem/) |
| **→ 后续** | [Ch19 分页](../chapter-19-paging/) — 每应用独立虚拟地址空间 |
