...
| 文档 | 内容 |
|------|------|
| [SETUP.md](../../SETUP.md) | Windows / WSL 编译环境；Ch1 LLVM；全书 EDK II 稍后 |
...
→ 环境 [SETUP.md](../../SETUP.md) · 元文件 [appendix-C](../../appendix-C-edk2-files/)

---

### 二、用 EDK II 重写 Hello World

Ch 1 已用 C + Clang/`lld-link` 写出 Hello World；本章改用 EDK II **基础库**：
...
若 Ch1 已走 **Clang + lld-link**，Ch2 可在 EDK II 里 **继续统一 LLVM**，不必切 GCC 交叉链。

| 配置 | 作用 |
|------|------|
| **`TOOL_CHAIN_TAG = CLANGPDB`** | MikanLoaderPkg / 平台 DSC 里指定 **Clang + LLD** 工具链 |
| **`build -t CLANGPDB`** | 命令行等价 — 构建产物目录常含 `CLANGPDB` 字样 |
| **前提** | 已 `source edksetup.sh`；PATH 有 **clang**、**lld-link** / lld |
