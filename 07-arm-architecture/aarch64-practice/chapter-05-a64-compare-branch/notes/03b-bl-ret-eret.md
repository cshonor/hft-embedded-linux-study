# 5.3b 跳转指令 — BL/BLR/RET/ERET

> 来源：§5.3 · 精读 · [← 条件跳转 B.cond](03-branch.md) · [章总览](section-0-本章完整概述.md)

## 本节讲什么

函数调用（BL/BLR）、函数返回（RET）、异常返回（ERET）、跳转范围限制、BL 与栈帧的关系。
条件跳转 B.cond 见 [03-branch.md](03-branch.md)。
...
> BL 把返回地址存入 X30(LR)。如果被调函数内部又 BL 其他函数，X30 会被覆盖 → 必须保存。
