...
CMP A, B 做的是 A - B，但 ALU 只有加法器。减法通过**取反加一**实现：
...
ALU 实际执行：x0 + (~x1) + 1
  ...
  +1  = 0xFFFFFFFFFFFFFFFD   ← 这就是 -3 的补码
  5 + 0xFFFFFFFFFFFFFFFD = 2 （产生进位 C=1）
...
1. **B 寄存器不变**：取反加一在 ALU 内部电路完成，x1 还是原来的 3
2. **同一套电路处理正负**：无论 B 是正还是负，ALU 都做相同的 `~B + 1`
   - B=3  → `~3+1 = -3` → A+(-3) = A-3 ✓
   - B=-3 → `~(-3)+1 = 3` → A+3 = A-(-3) ✓
3. **ALU 不判断符号**：补码的数学性质保证 `~B + 1` 对任何比特流都是正确的取负，不需要先检查正负

> 这就是补码设计的精妙——减法器不需要存在，加法器 + 取反加一 = 减法。而且不需要分支判断 B 的正负，同一套电路通杀。
