...
独占监视器有两种——本地监视器和全局监视器：

```
本地监视器（Local Monitor）：
  - 每个核一个，跟踪该核的 LDXR 地址
  - 只检测同一核内的 LDXR→STXR 配对
  - 粒度：以"被监视的地址"为单位
...
> 这就是为什么 LDXR 和 STXR 之间不能有太多操作——全局监视器随时可能被其他核的写操作清除。

### 为什么是 LDXR/STXR 两条指令——LL/SC 设计模式

```
AArch64 采用 LL/SC（Load-Linked / Store-Conditional）模式：
  LDXR = Load-Linked（加载并链接监视器）
  STXR = Store-Conditional（条件存储，可能失败）

为什么不做成单条原子指令（像 x86 的 LOCK XADD）？

  LL/SC 优势：
  1. 任意 RMW 操作都能实现——不只是加法，还可以 CAS、位操作等
     LDXR → 任意计算 → STXR（中间做什么由软件决定）
  2. 硬件简单——不需要为每种原子操作设计专用 ALU 电路
  3. 无锁进度保证——至少有一个核的 STXR 最终会成功

  LL/SC 劣势：
  1. 高争用时反复重试 → 延迟不可预测
  2. LDXR 和 STXR 之间的窗口可能被干扰
  3. 活锁风险（两核互相清除监视器）

  ARMv8.1 LSE 折中方案：
  → 保留 LDXR/STXR 兼容旧硬件
  → 新增 LDADD/CAS/SWP 等单条原子指令
  → 运行时检测 LSE 支持，选择更快的实现
```

> 这是 RISC 设计哲学的体现——用简单的指令组合实现复杂操作，而非为每种场景设计专用指令。LDXR/STXR 是"乐高积木"，LDADD 是"预制件"。
