...
| 后续模块 | 需要汇编直觉 |
|----------|--------------|
| [20 构建](../08-embedded-boot-build/) | U-Boot 启动早期、内核 `head.S` |
| [21 驱动](../09-device-drivers-dt/) | 中断入口、原子指令 `ldxr`/`stxr` |
| [04 LKD](../05-linux-kernel/) | 对照 x86 `syscall` ↔ ARM `svc` |

**顺序：** Smith 精读章（或跳过）→ [**奔跑吧 OUTLINE**](./aarch64-practice/OUTLINE.md) → [20 Mastering Embedded Linux Programming](../08-embedded-boot-build/)。
...
| [19 Hennessy](../19-computer-architecture/) Ch2 | MESI — ARM 同样适用 |
...
- [ ] 知道设备树为何取代 hard-coded 寄存器（→ [21](../09-device-drivers-dt/)）

**下一章：** [20 嵌入式 Linux 构建](../08-embedded-boot-build/)
