...
| 层 | 图中例子 | 你现在学什么 |
|----|----------|--------------|
| **User Space** | 桌面、C/Python、Shell | [01 C](../../01-c-language/) · [04 TLPI](../../04-linux-userspace-api/) · Labs **Phase A** |
| **Kernel** | 进程/内存管理、**设备驱动**、FS、网络栈；中断处理也在这里 | [07 内核](../../07-linux-kernel/) · [12 驱动](../../12-device-drivers-dt/) · Labs **Phase C** |
...
| 仓库模块 | 落在图的哪一层 |
|----------|----------------|
| 04 用户态 API、Phase A | 左：User Space |
| 07 内核地图、12 字符/平台驱动、`.ko` | 中：Kernel（本课主战场） |
| 00 架构、板级 GPIO/总线 | 右：Hardware |
| 18 DPDK / UIO·VFIO | **故意绕开**中间标准驱动路径（旁路），先把标准三层搞清再谈 |
