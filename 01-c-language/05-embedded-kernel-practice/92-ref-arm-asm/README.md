...
> **本书主线不在这里。** 原 3.1–3.5 / 3.9（指令/寻址/伪指令/异常）已物理删除，[2.6 内联汇编](../02-ch2-gnu-c-advanced/2.6-mixed-programming) 归入主线一、[6.17 GNU ARM 工具链](../06-ch6-toolchain-custom/6.17-gnu-arm) （原 3.7）归入主线二。
...
>
> **保留并提到 A 档**：**2.6 C/汇编混合编程**（`__asm__ __volatile__` 的操作数约束、clobber、volatile 语义全是**编译器层面**知识 = GNU C 扩展）、**6.17 GNU ARM 工具链**（原 3.7）（`.section` 与链接脚本联动、读懂 `objdump -dS`）、**3.8 AArch64**（选读）。
>
...
| 章节 | 内容 |
|------|------|
| **[ch01](../06-ch6-toolchain-custom/toolchain)** | `gcc`、`gdb`、`objdump`、`make`、交叉编译概念 |
| **[3.1 嵌入式 C 开门](../03-ch3-embedded-driver/3.1-register/3.1-寄存器操作.md)** | 读得懂 MMIO 的 C 表达；原 ch02 硬件内容已删 |
...
| **GNU 语法** | 3.7 | `.section` 直接对应 [ch04 链接脚本](../06-ch6-toolchain-custom/compile-and-link)；读懂 `objdump -dS` 是验证 C 优化的刚需 |
...
| **demo05_objdump** | `objdump -dS` 对照栈帧 | **6.17.7** |
...
5. `.section` 伪操作与链接脚本里的段如何对应（→ [ch04](../06-ch6-toolchain-custom/compile-and-link)）
...
- [6.17 GNU ARM汇编语言](../06-ch6-toolchain-custom/6.17-gnu-arm/6.17-GNU-ARM汇编语言.md)
  - [6.17.1 重新认识编译器](../06-ch6-toolchain-custom/6.17-gnu-arm/6.17.1-重新认识编译器.md)
  - [6.17.2 GNU ARM编译器的伪操作](../06-ch6-toolchain-custom/6.17-gnu-arm/6.17.2-GNU-ARM编译器的伪操作.md)
  - [6.17.3 GNU ARM汇编语言中的标号](../06-ch6-toolchain-custom/6.17-gnu-arm/6.17.3-GNU-ARM汇编语言中的标号.md)
  - [6.17.4 .section伪操作](../06-ch6-toolchain-custom/6.17-gnu-arm/6.17.4-section伪操作.md)
  - [6.17.5 基本数据格式](../06-ch6-toolchain-custom/6.17-gnu-arm/6.17.5-基本数据格式.md)
  - [6.17.6 数据定义](../06-ch6-toolchain-custom/6.17-gnu-arm/6.17.6-数据定义.md)
  - [6.17.7 汇编代码分析实战](../06-ch6-toolchain-custom/6.17-gnu-arm/6.17.7-汇编代码分析实战.md)
...
[3.1 嵌入式 C 开门](../03-ch3-embedded-driver/3.1-register/3.1-寄存器操作.md)。
