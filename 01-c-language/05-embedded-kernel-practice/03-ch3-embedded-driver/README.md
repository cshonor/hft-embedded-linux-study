...
| 本目录 | 原书位置 | 为什么归到这里 |
|--------|---------|--------------|
| [3.1 嵌入式 C 开门](./3.1-register/3.1-寄存器操作.md) | 原 10.8 寄存器操作 | 已改写成 C 语言视角（volatile / 屏障 / 位操作 / 字节序 / 未对齐） |
| [3.3 裸机多任务](./3.3-bare-metal/) | 原 10.1 | 裸机环境是嵌入式 C 的默认背景 |
| [3.2 中断](./3.2-interrupt/) | 原 10.3 | ISR 与主循环的共享数据保护，纯 C/并发问题 |
| ~~原 6.18 链接脚本~~ | 已移到 [CH6](../06-ch6-toolchain-custom/6.18-链接脚本.md) | section 属性 + 链接脚本 = 把代码钉到物理地址 |
| ~~原 3.7 GNU ARM 工具链~~ | 已移到 [CH6](../06-ch6-toolchain-custom/6.17-gnu-arm) | 交叉编译、objdump -dS 读反汇编 |
...
| 序 | 小节 | 状态 | 关键结论 |
|----|------|------|---------|
| 1 | [3.1 嵌入式 C 开门](./3.1-register/3.1-寄存器操作.md) | ✅ 29 KB | `-O2` 下不加 volatile 的反汇编**只剩 `jmp <自己>`**；volatile 不原子（20 万次 ×2 只得到 276660） |
| 2 | [6.18 链接脚本](../06-ch6-toolchain-custom/6.18-链接脚本.md) | 待扩 | `section` 属性如何把变量钉到指定段 |
| 3 | [3.2 中断](./3.2-interrupt/) | 待改造 | ISR 与主循环的共享数据：`volatile` + `__atomic_*` |
| 4 | [3.3 裸机多任务](./3.3-bare-metal/) | ✅ 已扩（2026-09-20） | 入口不是 `main`；`.bss` 不占 flash，全零 `const` 反占 flash（实测） |
| 5 | [3.7 GNU ARM 工具链](../06-ch6-toolchain-custom/6.17-gnu-arm) | 待扩 | 交叉编译 + `objdump -dS` 读反汇编 |

> 建议按 1 → 2 → 3 的顺序：先学会「跟硬件打交道时 C 该怎么写」，再看链接脚本把代码放哪，最后才是中断这种并发场景。

> **延伸阅读**：《裸机C编程：嵌入式系统C程序设计》（Stephen Oualline 著，黄俊彬译，机械工业出版社 2025-10，ISBN 9787111792017）。整本书的体量讲裸机专题，本仓库不展开复述——3.3 相关小节需要深挖时（编译器如何生成程序、中断例程写法、堆与缓冲 I/O）按其章节摘引。
...
|-------------|----------------|
| `volatile` 属性 | 寄存器轮询、ISR 共享标志（[3.1](./3.1-register/3.1-寄存器操作.md)） |
| `__attribute__((packed))` | 硬件寄存器映射结构体、网络报文头（[3.1](./3.1-register/3.1-寄存器操作.md)） |
| `__attribute__((aligned(n)))` | DMA 缓冲区、cache line 对齐（[3.1](./3.1-register/3.1-寄存器操作.md)） |
| `__attribute__((weak))` | 板级 hook、可选驱动（[2.4](../02-ch2-gnu-c-advanced/2.4-weak/2.4-属性声明-weak.md)） |
| `__attribute__((section(".x")))` | 把数据/代码钉到指定地址（[6.18](../06-ch6-toolchain-custom/6.18-链接脚本.md)） |
| `__builtin_bswap` / `ffs` / `popcount` | 字节序转换、位扫描（[3.1](./3.1-register/3.1-寄存器操作.md)） |
