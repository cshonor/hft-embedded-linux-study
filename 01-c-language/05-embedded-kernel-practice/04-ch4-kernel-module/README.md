...
| 序 | 小节 | 状态 | 内容 |
|----|------|------|------|
| 1 | [4.1 内核模块的加载与符号决议](./4.1-module-loading/4.1-内核模块加载与符号决议.md) | ✅ 38 KB | **`.ko` 是可重定位 ELF** / 内核当手写动态链接器（`load_module` 全流程）/ `EXPORT_SYMBOL` 两阶段（`.export_symbol`→`__ksymtab`）/ `resolve_symbol`+CRC / `apply_relocate_add` 真实公式（S+A / S+A-P）/ vermagic 门禁 / `__this_module` 指定初始化+alias 闭环 / 7 类内存布局与 `.init` 释放 |
...
本章目前 **4.1 模块加载 + 4.2 initcall + 4.3 container_of + 4.4 packed 实践已精写**（合计 126 KB），其余骨架状态。建议顺序：
...
| 内存信息来源 | 看 [CH6 链接脚本](../06-ch6-toolchain-custom/6.18-链接脚本.md) | `kmalloc` / `vmalloc` / slab |
...
| 目录 | 内容 | 对应小节 |
|------|------|------|
| [demo/4.2-initcall/](./demo/4.2-initcall/) | 11 个源文件 + 4 个链接脚本 + run.sh | [4.2 initcall 机制全解](./4.2-initcall/4.2-initcall机制全解.md) |
| [demo/4.1-module/](./demo/4.1-module/) | 5 个源文件 + t5_module.o + run.sh | [4.1 内核模块加载与符号决议](./4.1-module-loading/4.1-内核模块加载与符号决议.md) |
| [demo/4.4-packed/](./demo/4.4-packed/) | 5 个源文件 + run.sh | [4.4 协议结构体的 packed 实践](./4.4-packed-protocol/4.4-协议结构体的packed实践.md) |

> 所有 demo 可在 WSL 一键复现：`cd demo/4.2-initcall && bash run.sh`
