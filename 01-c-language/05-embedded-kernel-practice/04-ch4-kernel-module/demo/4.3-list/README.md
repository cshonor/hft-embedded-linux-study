# demo/4.3-list/ —— 内核链表机制全量实测

对应 [4.3 container_of 与侵入式链表](../../4.3-container-of-list/4.3-container_of与侵入式链表.md)。

## 运行

```bash
# 在 WSL 中 (注意: 用 wsl -d Ubuntu --exec 调用, 不要用 wsl -- bash -lc)
wsl -d Ubuntu --exec bash -c "cd /mnt/d/repos/hft/01-c-language/05-embedded-kernel-practice/04-ch4-kernel-module/demo/4.3-list && bash run.sh"
```
