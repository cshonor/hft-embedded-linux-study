...
| 扩展 | 典型写法 | 驱动里干什么 | 深挖 |
|------|----------|--------------|------|
| **指定初始化** | `{ .compatible = "a,b" }, { .probe = fn }` | `of_match_table`、`file_operations`、`platform_driver` | [6.2](./6.2-designated-init/) · [struct 驱动向](../../01-basics-syntax-and-types/ch06-structures/6.0-struct-for-drivers.md) |
| **`packed`** | `__attribute__((packed))` | 寄存器/协议结构体紧凑布局，忌乱对齐 | [6.7.4](../02-ch2-gnu-c-advanced/6.7-aligned/6.7.4-属性声明-packed.md) |
| **`section`** | `__attribute__((section("…")))` | 放到指定 ELF 段（init、特殊表） | [6.6](../02-ch2-gnu-c-advanced/6.6-section) |
...
| **`likely`/`unlikely`** | `__builtin_expect` | 分支预测提示 | [6.11.6](../02-ch2-gnu-c-advanced/6.11-builtin/6.11.6-Linux内核中的likely和unlikely.md) |
