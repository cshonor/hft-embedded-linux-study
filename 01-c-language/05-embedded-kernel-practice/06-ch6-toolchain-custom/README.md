...
> **配套**：utility 环境见 [1 工具链](./6.01-toolchain/)
...
| 序 | 目录 / 文件 | 原书位置 | 核心问题 |
|----|------------|---------|---------|
| 1 | [1 工具链](./6.01-toolchain/) | 原第 1 章 | 编辑器 / make / git / **ELF 分析工具** / gdb |
| 2 | [2 编译与链接](./6.07-compile-and-link/) | 原第 4 章 | 四阶段、静态库、动态库、装载与运行 |
| 3 | [6.17 GNU ARM 汇编与伪操作](./6.17-gnu-arm/) | 原 3.7 | **交叉编译**、`.section` 伪操作、`objdump -dS` 读反汇编 |
| 4 | [6.18 链接脚本](./6.18-链接脚本.md) | 原 4.14 | **把代码钉到指定地址**——本章最硬的一节 |
...
2 编译与链接      -> 默认流程是什么：预处理->编译->汇编->链接->装载
     |                + 静态库(.a) 与动态库(.so) 的本质差别
     |                + GOT / PLT / PIC（见 6.14-dynamic-linking）
     v
6.18 链接脚本     -> 默认流程不够用：我需要代码从 0x87800000 开始跑
     |                + PROVIDE / ENTRY / MEMORY / >region AT>region
     |                + __image_copy_start 这类符号怎么来
     v
6.17 GNU ARM      -> 换个目标就有了新约束：交叉工具链 + 汇编伪操作
                     + objdump -dS 验证「编译器到底优化成什么样了」
```

> **建议顺序**：2（搞懂默认） → 6.18（知道怎么改） → 6.17（换到交叉环境） → 1（缺什么补什么）。
...
| [CH3 volatile](../03-ch3-embedded-driver/3.1-register/3.1-寄存器操作.md) | `-O2` 下挥发性访问是否被合并；只能看反汇编 |
...
| `readelf -S` | 看段表（有哪些 section、地址、大小） | 6.18 链接脚本 |
| `objdump -dS` | 源码+汇编对照，**验证优化是否发生** | CH1 1.6 inline / CH3 3.1 volatile |
| `ld --verbose` | 打印默认链接脚本 | 6.18（写自定义脚本的模板） |
| `gcc -S` / `-E` | 看编译/预处理结果 | 2/6.9-compilation |
| `ar t` / `ar x` | 看静态库成员（**理解"成员提取"的前提**） | CH2 2.4 + 2/6.13 |
| `size` | 各段大小（嵌入式上斤斤计较） | 6.18 |
| `file` | 目标文件架构（交叉编译第一件事） | 6.17 |

## 衔接

- **应用层**（进程、线程、内存管理）不在本章：见 [附录 A OS 通识](../90-ref-os/) · [附录 B 内存堆栈](../91-ref-memory/)
- **构建产物怎么排错**：见 [2/6.10 链接过程](./6.07-compile-and-link/6.10-linking/)
...
<details><summary>答案</summary>
多数嵌入式目标机资源不足以跑编译器（几 MB 内存、无文件系统），而且没有标准库和目标头文件。**交叉编译器**在 x86 主机上跑，产出目标架构的机器码，配套的 sysroot 提供目标平台的头文件与库。所以命令名带了前缀——`arm-none-eabi-gcc` 里 `none` 表示无操作系统、`eabi` 表示嵌入式 ABI。详见 [6.17](./6.17-gnu-arm/)。
</details>
