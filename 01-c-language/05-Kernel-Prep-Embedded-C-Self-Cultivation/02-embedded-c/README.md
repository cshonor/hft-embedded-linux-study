...
**Embedded C in Practice** —— 把主线一的 GNU C 扩展真正用起来

> **本书第 5 本《嵌入式 C 语言自我修养》重排后的第二条主线**
> **前置**：[主线一 · GNU C 扩展](../01-gnu-c-extensions/)（`volatile`、`packed`、`weak`、位操作内建函数都在那儿）
...
所以本目录**一条硬件原理都不写**。Cache 一致性、字节序成因、MMU 页表、流水线冒险——这些交给 CSAPP。这里只写**它们在 C 里长什么样、写错了编译器会不会告诉你、以及怎么写才是对的**。

## 内容归属说明

本目录是从原书不同章节里**抽出来的嵌入式相关小节**，不是某一章的连续内容：
...
| [4.14 链接脚本](./4.14-链接脚本.md) | 原 4.14 | `section` 属性 + 链接脚本 = 嵌入式落地最硬的一节 |
| [3.7 GNU ARM 工具链](./3.7-gnu-arm/) | 原 3.7 | 交叉编译、`objdump -dS` 验证编译器优化 |

原书的进程/线程/文件系统/IO/MMU 留在 [09-os-reference](../09-os-reference/)（OS 通识，别处讲得更深）；ARM 指令集本身已删除。
...
| `__attribute__((weak))` | 板级 hook、可选驱动（[6.9](../01-gnu-c-extensions/6.9-weak/6.9-属性声明-weak.md)） |
| `__attribute__((section(".x")))` | 把数据/代码钉到指定地址（[4.14](./4.14-链接脚本.md)） |
| `__builtin_bswap` / `ffs` / `popcount` | 字节序转换、位扫描（[10.8](./10.8-register/10.8-寄存器操作.md)） |
| 柔性数组 | 变长报文、DMA 描述符（[6.5](../01-gnu-c-extensions/6.5-zero-length-array/6.5-零长度数组.md)） |
