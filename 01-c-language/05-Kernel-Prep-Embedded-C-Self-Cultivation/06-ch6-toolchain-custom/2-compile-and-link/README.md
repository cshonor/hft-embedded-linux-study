...
> [4.14 链接脚本](../4.14-链接脚本.md) 已归入主线二。这里保留编译/链接主体。
...
| 章节 | 内容 |
|------|------|
| **[ch01](../1-toolchain)** | `gcc`、`make`、`gdb`、`objdump` |
| ~~ch02 体系结构~~ | 🗑️ 已删，见 [主线二 10.8](../../03-ch3-embedded-driver/10.8-register/10.8-寄存器操作.md) |
...
```bash
cd 00-Linux-Kernel-DPDK-Network-C/05-Kernel-Prep-Embedded-C-Self-Cultivation/04-compile-and-link/demo
...
- [4.10 Linux内核模块运行机制](../../04-ch4-kernel-module/4.10-Linux内核模块运行机制.md)
- [4.11 Linux内核编译和启动分析](../../04-ch4-kernel-module/4.11-Linux内核编译和启动分析.md)
- [4.12 U-boot重定位分析](../../04-ch4-kernel-module/4.12-U-boot重定位分析.md)
