...
**GNU C Extensions for Kernel, Bootloader & DPDK** · 本书唯一核心
...
标准 C 教材不讲、但你读 **Linux 内核 / U-Boot / DPDK** 源码时每行都会撞见的东西：`typeof`、语句表达式、柔性数组、`__attribute__`、`weak`……

> 主线一讲**工具本身**；这些东西在什么场合该用，见 [主线二 · 嵌入式 C 落地](../02-embedded-c/)。

## 阅读顺序（不要按 6.1 → 6.12 的顺序）

先读这五篇，它们互相引用最多，构成本章骨架：

| 序 | 小节 | 状态 | 一句话 |
|----|------|------|--------|
| 1 | [6.6 `__attribute__` 总纲](./6.6-section/6.6.1-GNU-C编译器扩展关键字-__attribute__.md) | ✅ 37 KB | 属性语法总表，当字典翻 |
| 2 | [6.4 typeof 与 container_of](./6.4-typeof-container-of/6.4-typeof与container_of宏.md) | ✅ 28 KB | 宏能「看见」实参类型 |
| 3 | [6.3 语句表达式](./6.3-statement-expr/6.3-宏构造-利器-语句表达式.md) | ✅ 29 KB | 宏能「有局部作用域」；与 6.4 是两件套 |
| 4 | [6.5 零长度数组与柔性数组](./6.5-zero-length-array/6.5-零长度数组.md) | ✅ 39 KB | 变长报文 / DMA 描述符 |
| 5 | [6.7 aligned 与 packed](./6.7-aligned/6.7-属性声明-aligned.md) | ✅ 32 KB | 布局控制，接 6.5 |
| 6 | [6.9 weak 与 alias](./6.9-weak/6.9-属性声明-weak.md) | ✅ 31 KB | 唯一一个链接期生效的属性 |
| 7 | [6.10 inline](./6.10-inline/) | ▶ 下一批 | 与 weak 语义对立 |
| — | 6.11 内建函数 · 6.12 变参宏 · 6.2 指定初始化 · 6.1 C 标准 | 待扩 | — |
| 附 | [3.6 内联汇编](./3.6-mixed-programming/) | 待扩 | `__asm__ __volatile__` 本质是 GNU 扩展 |

**想看某一节原来属于第几章**（例如 `3.6` 来自原第 3 章），见 [书 README](../README.md) 的对照表。
...
| 章节 | 内容 |
|------|------|
| **[ch01](../03-toolchain/)** | `gcc`、`make`、基本命令行 |
| **[ch04](../04-compile-and-link/)** | 编译链接、ELF 段、符号表、`nm`/`readelf` |
| **[ch05](../05-memory-and-stack/)** | 堆 `malloc`、结构体布局、对齐概念 |
...
```bash
cd 00-Linux-Kernel-DPDK-Network-C/05-Kernel-Prep-Embedded-C-Self-Cultivation/01-gnu-c-extensions/demo
