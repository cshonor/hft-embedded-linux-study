...
引用：必须初始化、不能为空、不能改绑、使用时无需解引用（`r` 直接就是 `x` 的别名）。指针：可以不初始化、可以为 NULL、可以改指向、使用时需解引用（`*p`）。引用更安全（不会悬空），但指针更灵活（可表达「无对象」）。C 没有引用，只能用指针。C++ 的函数参数推荐用引用——避免拷贝且语法更自然。
