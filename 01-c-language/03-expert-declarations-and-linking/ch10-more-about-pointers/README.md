...
```bash
cd 00-Linux-Kernel-DPDK-Network-C/03-expert-declarations-and-linking/ch10-more-about-pointers/demo
...
> 再论指针：const 四组合、char* vs char[]、函数指针、void*。看代码 → 想答案 → 点开验证。
...
**答案：**
- 1 `*p1 = 5`：**不合法** — `const int *` 指向的内容不可修改
- 2 `p1 = &y`：**合法** — 指针本身可改指向
- 3 `*p2 = 5`：**合法** — `int *const` 指向的内容可修改
- 4 `p2 = &y`：**不合法** — 指针本身是常量

**口诀：** `const` 在 `*` 左边 → 指向常量；在 `*` 右边 → 指针常量；两边都有 → 都常量。

**复习：** → [10.4 向函数传递一个一维数组](./10.4-向函数传递一个一维数组.md)
...
> A 和 B 哪个安全？sizeof 分别是多少？

<details>
<summary>答案与复习指引</summary>

**答案：**
- A：**不安全（UB）** — `s1` 指向字符串字面量（`.rodata` 只读），修改 → 可能 SIGSEGV
- B：**安全** — `s2` 是栈上数组副本，内容可修改

- `sizeof(s1) = 8`（指针大小，64 位）
- `sizeof(s2) = 6`（5 字符 + `\0`）

**Iliffe 向量：** `char *lines[]` 各行指字面量（只读）；`char lines[][N]` 内嵌数据（可写）。

**复习：** → [10.2 指针数组就是 Iliffe 向量](./10.2-指针数组就是Iliffe向量.md)
...
**答案：**
- A `*vp`：**不合法** — `void*` 不能解引用（不知道大小）
- B `vp[0]`：**不合法** — 不能下标（本质也是解引用+算术）
- C `vp + 1`：**不合法**（C 标准不允许 `void*` 算术；GNU 扩展按 1 字节步长）
- D `int *ip = vp`：**合法**（C 允许 `void*` 隐式转 `T*`；C++ 需显式 cast）
- E `*ip = 42`：**合法** — `ip` 是 `int*`，可以解引用

**规则：** `void*` 只能存储、传递、转换——不能直接操作。`malloc` 返回 `void*` 赋给 `int*` 是合法的隐式转换。

**复习：** → [10.7 使用指针创建和使用动态数组](./10.7-使用指针创建和使用动态数组.md)
...
> 上面的释放代码有什么问题？正确写法是什么？

<details>
<summary>答案与复习指引</summary>

**答案：** **内存泄漏** — 只释放了指针数组，没释放每行数据。正确写法：
...
**顺序：** 先内后外（先行后表）。反过来会导致 `matrix[i]` 悬垂，无法释放各行。

**对比连续 `int[3][4]`：** 一次 `malloc` 一次 `free`，简单且 cache 友好——优先使用连续布局。

**复习：** → [10.3 在锯齿状数组上使用指针](./10.3-在锯齿状数组上使用指针.md)
