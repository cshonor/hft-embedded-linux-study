...
| **头文件 `inline`** | 函数体放头文件必须 **`static inline`**；裸 `inline` 不导出外部定义 → `undefined reference`（C99 语义；`gnu89` 下才是重复定义）（**5.1**） |
...
| **[01-basics-syntax-and-types](../../01-basics-syntax-and-types/)** | 函数、作用域、多文件编译基础 |
...
```bash
cd 00-Linux-Kernel-DPDK-Network-C/03-expert-declarations-and-linking/ch05-thinking-of-linking/demo
...
| 关联 | **[Embedded C](../../05-embedded-kernel-practice/)** 启动代码、链接脚本、裸机 |
