...
K&R 原书的小节顺序是：7.1 getchar/putchar → 7.2 printf → 7.3 变长参数 → 7.4 scanf → 7.5 FILE* → 7.6 stderr → 7.7 fgets → 7.8 其它函数。原 7.8「其它函数」是标准库杂物间（string/ctype/malloc/math/rand），与 stdio 无关，已删除；其中 `ungetc` 是真正的流操作，已并入 7.2 字符 I/O。
