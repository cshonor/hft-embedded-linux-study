...
> 底本：`../LEARNING-EBPF-BILINGUAL.pdf`。要自己写 eBPF，必须同时处理两半：内核侧 eBPF 程序 + 用户侧加载/附加/读数据的代码。本章是选型地图：bpftrace（一行搞定）→ BCC（Python 快速原型）→ libbpf / ebpf-go / libbpfgo / Aya（CO-RE 生产级）。
...
- 第 2 章 `02-HelloWorld.md`：BCC 第一个例子
- 第 5 章 `05-CO-RE一次编译处处运行.md`：BCC 五痛点、libbpf 骨架、`bpftool gen skeleton`
- 第 3 章 `03-eBPF程序解析.md`：字节码结构（本章练习建议用 llvm-objdump 对比）
- 第 4 章 `04-bpf系统调用.md`：`strace -e bpf` 观察库的加载行为
- 第 9 章 `09-eBPF安全.md`：TOCTOU——syscall 入口工具不能当安全防线
