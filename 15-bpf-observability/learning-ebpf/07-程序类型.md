...
> 底本：`../LEARNING-EBPF-BILINGUAL.pdf`。当前内核约 30 种程序类型、40+ 种附加类型。本章回答三个问题：程序类型决定什么？追踪类有哪些选型？网络类有哪些挂点？
...
- 前置：`05-CO-RE一次编译处处运行.md`（SEC 名、BPF_KPROBE_SYSCALL 宏、vmlinux.h）、`06-验证器.md`（helper 可用性检查、bpf_context access）
- 后续：`08-eBPF网络.md`（XDP/TC/socket filter 实战、uprobe 钩 SSL 明文）、`09-eBPF安全.md`（LSM、syscall 探针可被绕过的原因）
