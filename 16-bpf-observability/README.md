...
**文件夹 17** · 全书 **18 章 + 附录 A–E**（4 Parts）· [返回总清单](../READING-LIST.md#8-bpf-performance-tools--brendan-gregg)

> **文件夹 17** · 紧接 [16-Systems-Performance](../15-systems-performance/) · **同样后置**（见 [README](../README.md)）。  
> **建议时机：** 已有 Linux 内核/网络/DPDK 或 HFT 压测靶子后再开 — 用 BPF 验证真实系统。  
> **后续：** [18-HFT](../17-hft-engineering/) / [21-Rust](../21-rust-quant/)
...
与 [02-CSAPP](../02-computer-systems/) · [16-Systems-Performance](../15-systems-performance/) 同一套约定。
...
- **上一本（必读前置）** → [15-systems-performance](../15-systems-performance/) — 读完立刻读本目录
- SysPerf bpftrace 附录 → [02 appendix-C](../15-systems-performance/appendix-C-bpftrace单行命令.md)
- 后续内核/内存/网络 → [05-linux-kernel](../05-linux-kernel/) · [06-linux-mm](../06-linux-mm/) · [14-Rosen](../13-kernel-networking/)（读时可回头用 BPF 验证）
- DPDK 对照 → [15-DPDK](../14-dpdk/)
