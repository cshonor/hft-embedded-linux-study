...
> 学习 Phase：现在 Phase1–2 以读源码为主；**真编译** 建议放到 Phase4（LKD/LFS）或嵌入式 `20-UBoot-Kernel-Build` 时再上机。
...
嵌入式交叉编译另装 `aarch64-linux-gnu-gcc` 等 — 见 `20-UBoot-Kernel-Build`，此处先讲 **本机架构原生编译**。
