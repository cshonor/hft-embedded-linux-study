...
| **② 压测** | trace、阶梯 λ、**N=1,2,4,8… 吞吐** | 实测 knee、P99@ρ、**USL 拟合 α/β**、profile 得 **f** |
| **③ 实盘** | Prometheus | 修正模型；旺季前扩容/限流 |

**自动化：** 压测脚本代入 **f、N、α、β、λ、μ** → 阿姆达尔 + USL + 排队论一并输出 — 见 [阿姆达尔/USL 现成工具](./section-2.7-阿姆达尔与USL.md#现成工具填参数即算不必手推公式)。
...
- [ ] **脚本**：压测 → 自动算 S(N)、USL 拟合、ρ knee（[工具](./section-2.7-阿姆达尔与USL.md#现成工具填参数即算不必手推公式)）
