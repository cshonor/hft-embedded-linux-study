...
```
helloos.nas 里的源码              nasm -f bin 后 ipl.bin（512 B）
─────────────────────            ───────────────────────────────
...
```
① VS Code 写好 helloos.nas（汇编源码，文本）
        │
...
**要点：** NASM **只负责第 ② 步**（512 B）；**第 ③ 步** 是把 `ipl.bin` **贴到 1.44 MB 整盘的最开头** — 和 Day 1 在 HxD 里「先建大盘、再填前 512 字节」是一回事，只是今天用 **`dd` / Makefile** 自动化（见 [2.4](./section-2.4-Makefile-入门.md) · [Day 1 §1.4](../day-01-boot-asm/notes/section-1.4-加工润色.md)）。
...
| 阶段 | 典型语言 | 本课程对应 |
|------|----------|------------|
| **启动区 / IPL** | **汇编**（别无选择） | Day 1–2 · **`helloos.nas`** |
