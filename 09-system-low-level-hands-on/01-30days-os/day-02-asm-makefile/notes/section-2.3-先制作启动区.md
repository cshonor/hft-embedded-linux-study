...
**要点：** NASM **只负责第 ② 步**（512 B）；**第 ③ 步** 是把 `ipl.bin` **贴到 1.44 MB 整盘的最开头** — 和 Day 1 在 HxD 里「先建大盘、再填前 512 字节」是一回事，只是今天用 **`dd` / PowerShell / Makefile** 自动化（见 [2.4](./section-2.4-Makefile-入门.md) · [Day 1 §1.4](../day-01-boot-asm/notes/section-1.4-加工润色.md) · [code/README 拼盘命令](../code/README.md#iplbin-拼成-helloosimg)）。
...
**名字不同，是因为「角色」不同 — 不是自动改名，而是人工/脚本把 512 B 嵌进 1.44 MB 载体。**
...
**Windows（PowerShell）：** 见 [code/build-img.ps1](../code/build-img.ps1) 或 [code/README](../code/README.md)。
...
→ 工具链总览：[TOOLCHAIN.md · `.bin` vs `.img`](../../TOOLCHAIN.md#bin-是什么-f-bin-vs-f-elf-vs-img)
