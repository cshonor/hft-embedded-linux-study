...
> `05.5` 与 `05.6` 是**两本不同的书**，编号只是挨着：
> `05.5` = 现代内核特性（调度/RCU/ARM64/同步），`05.6` = Kaiwan《Linux Kernel Debugging》。
> 同理 `06.5` vs `06.6`：`06.5` 是现代 MM，**`06.6` 是 Brendan Gregg《Systems Performance》
> （性能方法论整本书），并不是「MM 的现代版」**。
> → 这三对编号**不存在合并关系**，合并等于把两本无关的书塞进一个目录。

### 按主题查：该翻哪一层

> 30 个高频主题全量扫描后的结论：**绝大多数主题主场清晰**，
> 少数几处已用双向链接打通。查东西按这张表走，不用两层都翻。
...
> ⚠️ **最容易踩的一个**：`06` Ch5「启动内存分配器」原书讲的是 **bootmem**，
> 而 bootmem 在 **v4.x 后已彻底移除**。想查取代它的 `memblock` 要去 **06.5**；
> `06` 那章里的 156 处 memblock 属于「旧书章节的现代补充」，不是主场。
>
