...
**调优直觉：** 热路径 **少 syscall、少拷贝、少阻塞**；内核旁路见 [13-DPDK](../../../13-dpdk/)、用户态栈。
...
**HFT：** 高频收包 → **硬中断 + NAPI + softirq** 占 CPU；`mpstat`/`perf` 里看到 `%soft` 高要往网络栈查。→ [06 内核网络 Ch 14 NAPI/RSS](../../../12-kernel-networking/)
...
- **isolcpus / 专用核**： housekeeping 与 hot path 分离 → [14-HFT ch05](../../../14-hft-engineering/chapter-05-os-kernel-tuning/README.md)
