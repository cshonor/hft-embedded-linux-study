...
> **HFT 路线定位：** ⚪ **后置可选** — 通用零基础启蒙用书；做量化/服务器底层请 **直接 [01 MikanOS](../mikanos/)** → [HFT-AND-EMBEDDED-PRIORITY.md](../HFT-AND-EMBEDDED-PRIORITY.md)
...
> **⚠ HFT / 嵌入式主线：** 本书 **不是** 你的必读书 — **16 位 BIOS / 软盘与 x86-64 Linux 割裂**，学完对工作复用低。请 **C 吃透后直接 [01 MikanOS](../mikanos/)**；本书仅当休闲拓展。理由与固定顺序 → **[HFT-AND-EMBEDDED-PRIORITY.md](../HFT-AND-EMBEDDED-PRIORITY.md)**
...
**HFT 主线：** 本目录 **后置可选**；同等概念在 [01 MikanOS](../mikanos/) 用 **64 位现代架构** 学更高效。
...
与 [14-Systems-Performance](../../19-systems-performance/) / [07-linux-kernel](../../07-linux-kernel/00_Book_3rd_Notes/) 一致：每个 Day 独立目录，导读 + 分段笔记。
...
**HFT 最简路径（走 02，不走 01）：** C 扎实 → [02 MikanOS](../mikanos/) → LKD / DPDK。  
...
- **不必先备：** 完整 OS 理论课；**需要：** 基本 C + 简易汇编（与 [01-CSAPP Ch3](../../02-computer-systems/chapter-03-machine-level-programs/) 互补）
...
- [ ] 与 [07-linux-kernel](../../07-linux-kernel/) Ch4/7/8 对照表
...
| [01-CSAPP Ch3/5/8](../../02-computer-systems/) | 机器码、缓存、异常与进程（**HFT 主攻**） |
| [07-linux-kernel](../../07-linux-kernel/) | 中断、调度、内存管理 **真实实现** |
| [04-linux-userspace-api](../../04-linux-userspace-api/) | 用户态 API 与内核边界（**可与本书并行**） |
| [mikanos](../mikanos/) | 现代 UEFI/64 位 OS — **01 通读后的推荐下一站** |
