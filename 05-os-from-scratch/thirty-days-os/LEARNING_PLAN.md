...
> **HFT 路线：** 本文档面向 **通用零基础启蒙**；做 HFT/服务器底层请 **跳过本书，直接 [01 MikanOS](../mikanos/)** → [HFT-AND-EMBEDDED-PRIORITY.md](../HFT-AND-EMBEDDED-PRIORITY.md)
...
**若你已有 C++/体系结构背景：** C 与内存布局通常不是门槛，主要补齐 **16 位汇编 + NASM + make** 即可。可与 [01-CSAPP Ch3](../../02-computer-systems/chapter-03-machine-level-programs/) 并行。
...
> **HFT / 嵌入式主线：** **跳过本书**，C 达标后 **直接 [01 MikanOS](../mikanos/)** — 见 **[HFT-AND-EMBEDDED-PRIORITY.md](../HFT-AND-EMBEDDED-PRIORITY.md)**。  
> 下文面向 **选择阅读 01 的通用零基础路线**。

```
【通用】02 30天 OS（引导、中断、页表、任务切换）→ 01 MikanOS → 05 LKD → 16 HFT
【HFT】  C → 01 MikanOS → 05 LKD / 14 DPDK → 16 HFT  （跳过 thirty-days-os）
```

| 原书机制 | 仓库对照 |
|----------|----------|
| GDT/IDT/中断 | [07-linux-kernel Ch7–8](../../07-linux-kernel/00_Book_3rd_Notes/chapter-07-interrupts/) |
| 调度/上下文切换 | [07-linux-kernel Ch4](../../07-linux-kernel/00_Book_3rd_Notes/chapter-04-process-scheduling/) |
| INT 0x40 API | [07-linux-kernel Ch5 syscall](../../07-linux-kernel/00_Book_3rd_Notes/chapter-05-system-calls/) · [04-linux-userspace-api](../../04-linux-userspace-api/) |
