## 6. 信号量 (Semaphores) 与完成变量 (Completions)

---

### 一、信号量

与自旋锁不同：资源不可用时，当前进程 **挂起睡眠 (blocking wait)**，资源就绪后 **唤醒**。

| 变种 | 说明 |
|------|------|
| 计数信号量 | 允许多个持有者（有限资源池） |
| **读/写信号量** | 多读单写 — 提高读并发 |

适合：**仅被异常/系统调用路径访问**、临界区可能较长、**可以睡眠** 的场景。

→ 等待队列：[Ch 3](../chapter-03-processes/notes/section-4-organization-and-lookup.md)

---

### 二、完成变量 (Completions)

- 类似信号量，专用于 **「一件事完成了」** 的同步  
- 解决 SMP 上 **临时信号量动态分配/释放** 的竞态  
- 典型：`complete()` / `wait_for_completion()`

---

### 三、自旋锁 vs 信号量（性能直觉）

| 维度 | 自旋锁 | 信号量 |
|------|--------|--------|
| 等待成本 | 烧 CPU | 让出 CPU |
| 临界区长度 | 必须短 | 可较长 |
| 上下文 | 中断里 **只能用 spinlock** | 中断里 **不能** sleep → 不用 semaphore |
| HFT 热路径 | 首选（短临界区） | 一般避开 |

---

← [5. 顺序锁与 RCU](./section-5-seqlocks-and-rcu.md) · 下一节 [7. 选型与实例](./section-7-selection-and-examples.md)
