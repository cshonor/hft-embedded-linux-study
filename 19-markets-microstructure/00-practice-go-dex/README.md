...
> **父模块：** [19-markets-microstructure](../README.md)（Larry Harris · *Trading and Exchanges*）  
...
| | **上层 `19-markets-microstructure/`** | **本目录 `00-practice-go-dex/`** |
|---|--------------------------------------|----------------------------------|
| 内容 | **按原书章节** 的读书笔记（Ch 1–29 平铺） | **练手项目** 的设计笔记、里程碑、**Go 源码** |
| 排序 | 与 Harris **目录顺序一致** | 与 **实现进度** 一致（订单 → LOB → 撮合…） |
| 模块边界 | 市场微观 **业务语言** | 与 `05` 自制 OS、`05` Linux 内核等 **系统模块清晰分开** |
...
07 自制 OS / 05 LKD …（系统与内核 · 不同大模块，互不混放）
    ↓ 后期
14 HFT 工程 · 17 Rust 量化（网关/撮合热路径 → Rust；Go 留外围；业务模型仍来自 00）
```
