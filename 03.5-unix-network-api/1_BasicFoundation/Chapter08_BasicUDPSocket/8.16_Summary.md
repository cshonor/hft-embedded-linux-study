...
UDP 服务器比 TCP 简单（无连接管理）但问题更多：丢包、验源、异步错误、无流控。

**解法清单**：超时（alarm/select/SO_RCVTIMEO）、验源（比较地址/UDP connect）、异步错误（connect）、流控（应用层 ACK）。

> HFT 关联：UDP 低延迟无连接开销，适合行情分发；但可靠性全靠应用层补。
