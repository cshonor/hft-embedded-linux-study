...
运行时查连接本地/远端地址：`getsockname` 查本地，`getpeername` 查远端。

**三个场景**：bind 端口 0 查临时端口；bind INADDR_ANY 查实际网卡；inetd 子进程查客户身份。

> 参数仍是值-结果 socklen_t*（Ch3.3）。对 listenfd 调 getpeername 无意义。
