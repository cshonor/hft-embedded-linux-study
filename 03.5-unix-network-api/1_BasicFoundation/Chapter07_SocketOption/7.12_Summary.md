...
套接字选项是调内核协议栈行为的旋钮。高频五件套：REUSEADDR（重启用）、KEEPALIVE（保活）、RCVBUF（缓冲）、LINGER（close行为）、NODELAY（禁Nagle）。

**继承规则**：connfd 继承 listenfd 选项 → listen 前设在 listenfd 上。

> fcntl 设非阻塞是通向 Ch16 非阻塞 I/O 的入口。
