...
`fcntl(sockfd, F_SETFL, flags | O_NONBLOCK)` — 设置套接字非阻塞。

**三步**：F_GETFL 取当前 → 位或 O_NONBLOCK → F_SETFL 写回。不能直接 F_SETFL = O_NONBLOCK（会清其他标志）。

> HFT 关联：非阻塞 I/O + epoll = Linux 高并发标准模型。DPDK 绕过这层直接轮询。
