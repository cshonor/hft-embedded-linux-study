...
网络编程的核心模型就是 C/S：客户主动请求，服务器被动响应。UNP 全书都在这个模型上教 socket API。

**一句话**：两层直连库 → 三层加应用服（UNP socket）→ B/S 换浏览器走 HTTP。

> HFT 关联：交易所行情系统是典型 C/S——网关为服务器，策略进程为客户，走 TCP/UDP 而非 HTTP。
