...
Ch6 解决了 Ch5 暴露的"阻塞 I/O 天花板"——select/poll 让单进程同时监控多 fd。

**核心三步**：注册关心的 fd → 等待就绪 → 逐个处理就绪 fd。

**演进**：select(1024上限) → poll(无上限但O(N)) → epoll(O(1)就绪列表) → DPDK(轮询零syscall)。

> HFT 关联：epoll+非阻塞 I/O 是 Linux 网络编程标配；DPDK 连 epoll 都绕过。
