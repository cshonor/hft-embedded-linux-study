...
数据链路层访问方案：BSD BPF、Solaris DLPI、Linux PF_PACKET。libpcap 统一封装抓包，libnet 封装发包。

**核心概念**：BPF 过滤器在内核运行减少拷贝；UDP 校验和含伪首部。

**演进**：BPF → eBPF/XDP（Linux 高速包处理）；PF_PACKET → DPDK（零拷贝线速）。

> HFT 关联：DPDK 绕过所有内核层直接操作网卡——比 PF_PACKET + TPACKET 更快。
