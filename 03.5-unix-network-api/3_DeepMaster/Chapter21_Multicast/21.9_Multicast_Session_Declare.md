...
SAP（Session Announcement Protocol）= 在固定组播组 224.2.127.254:9875 上宣告组播会话。SDP（Session Description Protocol）描述会话内容。

**一句话**：SAP/SDP 是组播会话的"目录服务"——自动发现有哪些组播会话可用。

> HFT 不用 SAP/SDP——行情组播地址和端口是预配置的。
