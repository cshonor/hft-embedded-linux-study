...
ioctl 是历史遗留的"万能控制接口"——类型不安全、可移植性差，但大量功能仍依赖它。

**核心功能**：SIOCGIFCONF（接口列表）、FIONREAD（待读字节）、SIOCGIFFLAGS（接口状态）。

**趋势**：路由→AF_ROUTE/netlink；接口→getifaddrs；非阻塞→fcntl。ioctl 逐渐被替代但未消亡。

> HFT 关联：DPDK 完全不用 ioctl——有自己的 PMD 驱动直接操作网卡寄存器。
