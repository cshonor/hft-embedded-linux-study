...
高级 SCTP 概述：涵盖自动关闭、部分交付、通知机制、无序数据、地址子集绑定、关联拆分、心跳配置、定时器调优等高级特性。用于优化 SCTP 在生产环境的可靠性。

**一句话**：基础 SCTP=建连收发，高级 SCTP=通知/部分交付/心跳/拆分——精细控制关联行为。

> 易考点：高级特性大多通过 setsockopt 的 SCTP_* 选项配置，按 assoc_id 可针对单个关联。
