...
> **CSAPP 全书基准：** 3rd/后续版汇编、栈、调用、链接、缓存例子，默认都是 **x86-64（AMD64）+ Linux gas（AT&T）+ System V ABI**。  
> 截图里的数据格式表与操作数寻址 = 这本书 **唯一系统讲解** 的机器级规范；习题/实验按这套做。**与 ARM32 语法不通用**（对照见下与 [Smith §2.2](../../../19-ARM64-Architecture/arm32-smith-assembly/chapter-02-programmers-model/notes/section-2-2-data-types.md)）。
...
2. `int` / 指针后缀？前 6 个参数寄存器？（ARM32 指针几字节？）  
3. `$0x4` vs `0x4`？`(%rax)` / `8(%rbp)` 各等价什么？  
4. 为何写 `%eax` 会影响 `%rax` 高半？ARM32 有 `%al` 这类别名吗？  
5. `addq 8(%rbp), %rax` 在 ARM 上为何不能一条指令做完？  
6. Ch4 的 Y86-64 和本章真 x86-64 什么关系？
