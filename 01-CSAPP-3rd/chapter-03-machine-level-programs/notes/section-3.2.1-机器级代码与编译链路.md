...
| ❌ | 编译更慢、二进制更大；gdb 难跟；极少数浮点语义变化要小心 |
...
**HFT 规范（简）：** 日常学/调 → `-O0` / **`-Og`**；交易核心 Release → **`-O3`**（+ 与生产一致的 `-march`/LTO/PGO）。细节与编译器能/不能做 → [Ch5 §5.1](../../chapter-05-optimizing-performance/notes/section-5.1-优化编译器的能力和局限性.md)
