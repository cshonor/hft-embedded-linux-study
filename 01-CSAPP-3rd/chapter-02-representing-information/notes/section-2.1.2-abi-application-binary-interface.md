...
| 3 | **函数调用约定** | System V：前 6 整型 → `%rdi…%r9`，返回 `%rax`；Windows x64 寄存器**全不同** → 跨 ABI 调函数拿错参 → Ch3 §3.7 · **全景 → §6** |
...
整型参数依次：**`%rdi, %rsi, %rdx, %rcx, %r8, %r9`**（再溢出到栈）；返回值 **`%rax`**。  
换 Windows x64 调用约定 → 前几个参数改走 **`%rcx, %rdx, %r8, %r9`** 等 — **同 CPU、不同协议**，二进制不能直接互调。
...
切到另一套对齐规则（如 ILP32 下 `long` 只按 4 对齐，或 LLP64 下 `long` 本身只有 4 字节），**补的字节数、成员偏移、总大小全变**。

**同一段机器码、不同 ABI 的偏移灾难：**

| 谁写的二进制 | 读 `b` 怎么取 |
|--------------|----------------|
| 按 **LP64** 编出来的 | 基址 **+8**，取 **8** 字节 |
| 按另一套对齐（如 `b` @ **+4**）编/解读 | 基址 **+4** 去取 |

同一块内存按 LP64 排好了 `b` 在 +8；对方却用「+4」那套偏移去读 → **读出的值全错**；若目标 ABI/CPU 还要求严格对齐，还可能 **未对齐访问直接崩溃**。跨 ABI 把结构体当地址/二进制互传，正是第 2 项对齐规则让调用易炸的核心。

### 1 · 函数调用约定（CSAPP 必考 · ABI 第 3 项）

与第 2 项结构体对齐是 **配套规则**：一边管「内存怎么排」，一边管「调函数时参数/返回值走哪」。

**最直观：x86-64 System V（Linux）**

| | 规则 |
|--|------|
| 前 6 个整型/指针参数 | 依次 **`%rdi, %rsi, %rdx, %rcx, %r8, %r9`** |
| 第 7 个起 | **压栈** |
| 整型/指针返回值 | **`%rax`** |

**浮点参数：** `%xmm0`–`%xmm7`（与 GPR **分开计数**）。

**换一套就炸：Microsoft x64（Windows）对照**

| | System V（Linux） | Microsoft x64（Windows） |
|--|-------------------|--------------------------|
| 寄存器传参（整型） | 前 **6**：`rdi, rsi, rdx, rcx, r8, r9` | 前 **4**：`rcx, rdx, r8, r9` |
| 返回值 | `%rax` | `%rax`（同名，但前面参数槽已不同） |
| 额外习惯 | — | 调用方常预留 **32B shadow space** 等 |

同一段「调用方把第 1 参放进 `%rdi`」的机器码，对上 Windows 编出来的被调函数（它在 `%rcx` 里找第 1 参）→ **拿错参数，直接崩溃或逻辑全错**。这就是第 3 项调用约定的实际影响。
...
| ABI | 典型布局直觉 |
|-----|----------------|
| **Linux LP64** | `a` + 7 pad + `b`(8) → **16**；读 `b` = 基址 **+8**，取 8 字节 |
| **ILP32** | `long` 常 4 字节、按 4 对齐 → `b` 常在 **+4**；读法变成基址 **+4** |
| **Windows LLP64** | `long` 常 4 字节 → 与 Linux x64 **同 CPU 仍可能布局不同** |

**关键：** 不是「换了 CPU」，而是 **同一段机器码里的常量偏移**（`8(%rdi)` vs `4(%rdi)`）绑死了某套 ABI。按 LP64 排好的内存，却用 +4 那套规则去读 → 值全错；严格对齐架构上还可能 **未对齐访问崩**。
...
2. System V 前 6 个整型参数寄存器？浮点用哪组？返回值在哪？  
