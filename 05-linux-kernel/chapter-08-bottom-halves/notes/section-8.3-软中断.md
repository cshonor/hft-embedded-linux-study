...
#### 术语防撞：「软中断」这个词有两个完全不同的含义

中文资料最大的坑——**两个不相干的东西都翻译成"软中断"**：

| | CPU 教材的 software interrupt（**软件陷阱 trap**） | Linux 内核的 **softirq**（本文主角） |
|--|---------------------------------------------------|----------------------------------------|
| 是什么 | CPU 执行特殊指令产生的**同步异常** | 硬中断的**延后处理机制**（下半部） |
| 怎么触发 | 执行到那条指令才触发（`syscall` / `svc #0`） | 内核代码调 `raise_softirq()` 纯软件标记，**无陷入指令** |
| 上下文 | **进程上下文**（syscall）或异常处理 | **中断上下文** |
| 能否睡眠 | syscall ✅ 可以 | ❌ 禁止（也不能 `copy_from_user`） |
| 包含关系 | **系统调用 ⊂ 软件陷阱**（trap 还含缺页、除零、调试断点——syscall 只是其中"主动"的那类） | 与系统调用**毫无关系** |

> 两句话背诵：**系统调用是 CPU 的软件陷阱（同步异常），进程上下文，可以睡眠；Linux softirq 是硬件中断的延后处理，中断上下文，不能睡眠，和系统调用无关。**
> 联想键盘例子：按键 = 硬中断（中断上下文，只入队）；`read()` = 软件陷阱（进程上下文，`copy_to_user`）；网卡收包解析 = softirq（中断上下文，顶半部 `raise_softirq` 后延后跑）——三种机制在一条链上各干一段。
...
→ [Ch 8.6](section-8.6-ksoftirqd-辅助线程.md) ksoftirqd · [Ch 8.4](section-8.4-tasklet.md) tasklet 对比 · [12 Rosen Ch14 NAPI/softirq](../../../12-kernel-networking/chapter-14-advanced-topics/)
