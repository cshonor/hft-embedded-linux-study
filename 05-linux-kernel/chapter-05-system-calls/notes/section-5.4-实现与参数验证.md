...
`buf` 是用户态虚拟地址；内核若直接访问，可能触发页错误，严重时 **Oops**。
...
这两个函数会：校验用户地址合法性、配合缺页处理、安全复制缓冲区。  
...
`SYSCALL_DEFINE*` 是内核宏：自动完成函数包装、参数提取，最终注册进系统调用表。
...
> ↔ [ULK Ch10 §6 参数验证与内核封装](../../../18-linux-kernel-deep/chapter-10-system-calls/notes/section-6-参数验证与内核封装.md)
