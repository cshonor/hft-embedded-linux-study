...
**❌ 误区：**「内核有一条总的运行队列，分给各 CPU 跑」——不存在。Linux **不是全局队列模型**；也没有「CPU 的队列 vs 内核的队列」之分——`rq` 结构体存在内核内存里，但**每颗 CPU 只拥有并只看自己那条**。

```
内核（软件，调度器是其中一个模块）
├── CPU0 ──► struct rq ──┬── cfs_rq  普通任务红黑树
│                        ├── rt_rq   SCHED_FIFO/RR
│                        └── dl_rq   截止时间任务
└── CPU1 ──► struct rq ──┬── cfs_rq
                         ├── rt_rq
                         └── dl_rq
```
...
| 调度器职责 | 动作 |
|------------|------|
| **入队** | fork/clone 出新 task 后，**选一个 CPU**，把 task 挂到该 CPU 的 rq |
| **挑选** | 每颗 CPU 触发调度时，**只从本地 rq** 选下一个任务——不读别的 CPU 的队列 |
| **迁移** | 负载不均时，把 task 从 CPU0 的 rq 摘下、挂到 CPU1 的 rq（负载均衡） |

> `task_struct` 本身是**内核全局保存**的（所有 CPU 都能访问）；但「现在准备在哪跑」由**挂在哪条 rq 上**决定——迁移就是换挂的队列，`task_struct` 不动窝。

工厂比方：内核=工厂管理系统，CPU=工人，rq=**每个工人自己手上的待办清单**。工人只看自己的清单；工厂看到 A 活太多 B 闲着，就把任务从 A 的清单挪到 B 的——没有一张全世界统一的大清单。
