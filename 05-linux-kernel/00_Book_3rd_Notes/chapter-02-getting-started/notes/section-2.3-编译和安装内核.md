...
> 学习 Phase：现在 Phase1–2 以读源码为主；**真编译** 建议放到 Phase4（LKD）或嵌入式 `08-embedded-boot-build` 时再上机。
...
嵌入式交叉编译另装 `aarch64-linux-gnu-gcc` 等 — 见 `08-embedded-boot-build`，此处先讲 **本机架构原生编译**。
...
| [08-embedded-boot-build](../../../../08-embedded-boot-build/) | 嵌入式构建、交叉编译 |
